...
EUCONS = ROOT / "eucons"
DEFAULT_CONTRACT = EUCONS / "social" / "linkedin_contract.json"


class LinkedInAdapterError(ValueError):
    pass
...
def _knowledge_index(knowledge: dict[str, Any], contract: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if knowledge.get("product") != contract["input"]["required_product"]:
        raise LinkedInAdapterError("unknown knowledge product")
    if knowledge.get("engine_id") != contract["input"]["required_knowledge_engine_id"]:
        raise LinkedInAdapterError("unknown knowledge engine")
    if knowledge.get("runtime_publication_enabled") is not False:
        raise LinkedInAdapterError("LinkedIn adapter requires E14 runtime publication disabled")

    index: dict[str, dict[str, Any]] = {}
    for row in knowledge.get("records") or []:
        rid = str(row.get("id") or "").strip()
        if not rid or rid in index:
            raise LinkedInAdapterError("knowledge records require unique non-empty ids")
        index[rid] = row
    return index
...
def _canonical_url(record: dict[str, Any], contract: dict[str, Any]) -> str:
    ...
    base = contract["presentation"]["canonical_base"].rstrip("/")
    templates = contract["presentation"]["route_templates"]
    if rtype == "OPPORTUNITY":
        return base + templates["OPPORTUNITY"].format(source_ref=source_ref)
    if rtype == "CASE":
        return base + templates["CASE"].format(source_ref=source_ref)
    if rtype in {"GUIDE", "ANALYSIS", "FAQ"}:
        return base + templates[rtype].format(source_ref=source_ref)
    raise LinkedInAdapterError("unknown LinkedIn record type")


def _cta(record: dict[str, Any], contract: dict[str, Any]) -> str:
    return contract["presentation"]["cta_by_type"][str(record.get("type") or "")]
...
def _body(record: dict[str, Any], canonical_url: str, contract: dict[str, Any]) -> str:
    ...
    if not title or not summary:
        raise LinkedInAdapterError("LinkedIn READY record requires title and summary")

    parts = [title, "", summary]
    if rtype == "ANALYSIS":
        label = str(record.get("analysis_label") or "").strip()
        if not label:
            raise LinkedInAdapterError("analysis record requires analysis label")
        parts.extend(["", label])
    if rtype == "OPPORTUNITY":
        facts = record.get("material_facts") or {}
        if not isinstance(facts, dict) or not facts:
            raise LinkedInAdapterError("opportunity requires material facts")
        if not (record.get("verified_fact_classes") or []):
            raise LinkedInAdapterError("opportunity requires verified fact classes")

    parts.extend(["", _cta(record, contract), canonical_url])
    body = "\n".join(parts).strip()
    if len(body) > int(contract["presentation"]["max_body_chars"]):
        raise LinkedInAdapterError("LinkedIn body exceeds deterministic maximum")
    if "#" in body and contract["presentation"]["hashtags_default"] is False:
        raise LinkedInAdapterError("hashtags are disabled by default")
    return body


def _item_id(editorial_id: str, content_hash: str) -> str:
    basis = f"{editorial_id}|{content_hash}|linkedin"
    return "LI-" + hashlib.sha256(basis.encode("utf-8")).hexdigest()[:24]
...
def _receipt(item: dict[str, Any]) -> dict[str, Any]:
    ...
    body["receipt_id"] = "RCP-E17-" + hashlib.sha256(stable_json(body).encode("utf-8")).hexdigest()[:24]
...
def build_outbox(editorial: dict[str, Any], knowledge: dict[str, Any], contract: dict[str, Any]) -> dict[str, Any]:
    ...
    for decision in editorial.get("decisions") or []:
        ...
        if not editorial_id or editorial_id in seen_editorial:
            raise LinkedInAdapterError("READY editorial decisions require unique ids")
        seen_editorial.add(editorial_id)
        record = knowledge_by_id.get(knowledge_id)
        if not record:
            raise LinkedInAdapterError("READY editorial decision missing knowledge record")
        if record.get("publication_state") != "PUBLISHABLE":
            raise LinkedInAdapterError("READY editorial decision points to non-publishable knowledge")
        if record.get("type") not in contract["input"]["allowed_types"]:
            raise LinkedInAdapterError("unknown LinkedIn content type")
        if decision.get("source_ref") != record.get("source_ref") or decision.get("type") != record.get("type"):
            raise LinkedInAdapterError("editorial/knowledge identity mismatch")

        content_hash = str((decision.get("fact_kernel") or {}).get("content_hash") or "").strip()
        if not content_hash:
            raise LinkedInAdapterError("READY editorial decision missing content hash")
    ...
    return {
        ...
        "summary": {
            ...
            "published": 0,
            "held_for_external_authorization": len(items),
        },
        "items": items,
        "receipts": receipts,
    }
...
def assert_output_path_safe(path: Path) -> None:
    try:
        ...
        return
    raise LinkedInAdapterError("runtime LinkedIn outbox cannot be written under repository root")
