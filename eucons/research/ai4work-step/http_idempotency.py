...
def prepare_http_submission(payload: Any, idempotency_key: Any) -> tuple[dict[str, Any], str]:
    ...
    returned body digest is transport/storage metadata and must not be exported
    to NF06.
    """
    key = validate_idempotency_key(idempotency_key)
