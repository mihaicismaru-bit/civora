...
if ($processed['lead']['submitted_at'] !== '') fail_test('server time must not mutate idempotency payload');
...
$reprocessed = $runtime->process($payload);
$replay = $runtime->persist($reprocessed);
if ($replay['status'] !== 'accepted' || $replay['idempotent_replay'] !== true) fail_test('independent idempotent replay failed');
if ($replay['request_id'] !== $receipt['request_id']) fail_test('idempotent request id drift');
$changed = $payload;
$changed['message'] = 'Synthetic E29 PHP runtime changed payload.';
$changedProcessed = $runtime->process($changed);
expect_exception(fn() => $runtime->persist($changedProcessed), 'SUBMISSION_ID_CONFLICT');
