...
CONTRACT = json.loads((ROOT / "eucons" / "social" / "linkedin_contract.json").read_text(encoding="utf-8"))
ADAPTER_PATH = ROOT / "eucons" / "social" / "linkedin_adapter.py"


def load_adapter():
    spec = importlib.util.spec_from_file_location("eucons_linkedin_adapter_failclosed", ADAPTER_PATH)
...
BASE_EDITORIAL = {
    ...
    "decisions": [{
        "editorial_id": "EDT-1", "knowledge_id": "KNW-1", "type": "ANALYSIS", "source_ref": "SRV-01",
        "decision": "READY", "fact_kernel": {"content_hash": "a" * 64},
    }],
}


def must_fail(name: str, editorial=None, knowledge=None, contract=None) -> None:
    try:
        MODULE.build_outbox(
            copy.deepcopy(editorial if editorial is not None else BASE_EDITORIAL),
            copy.deepcopy(knowledge if knowledge is not None else BASE_KNOWLEDGE),
            copy.deepcopy(contract if contract is not None else CONTRACT),
        )
    except MODULE.LinkedInAdapterError:
        return
    raise SystemExit(f"{name}: adapter failed open")


def main() -> None:
    editorial = copy.deepcopy(BASE_EDITORIAL)
    editorial["dispatch_enabled"] = True
    must_fail("upstream dispatch enabled", editorial=editorial)

    editorial = copy.deepcopy(BASE_EDITORIAL)
    editorial["decisions"][0]["source_ref"] = "OTHER"
    must_fail("identity mismatch", editorial=editorial)

    editorial = copy.deepcopy(BASE_EDITORIAL)
    editorial["decisions"][0]["knowledge_id"] = "MISSING"
    must_fail("missing knowledge", editorial=editorial)

    knowledge = copy.deepcopy(BASE_KNOWLEDGE)
    knowledge["records"][0]["publication_state"] = "HOLD"
    must_fail("non-publishable knowledge", knowledge=knowledge)

    knowledge = copy.deepcopy(BASE_KNOWLEDGE)
    knowledge["records"][0]["analysis_label"] = ""
    must_fail("analysis label missing", knowledge=knowledge)

    knowledge = copy.deepcopy(BASE_KNOWLEDGE)
    knowledge["records"][0]["summary"] = "X" * 3000
    must_fail("oversized body", knowledge=knowledge)

    knowledge = copy.deepcopy(BASE_KNOWLEDGE)
    knowledge["runtime_publication_enabled"] = True
    must_fail("knowledge runtime enabled", knowledge=knowledge)

    print("EUCONS E17 LinkedIn fail-closed regressions: PASS")
