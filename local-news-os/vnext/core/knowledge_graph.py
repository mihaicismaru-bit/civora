...
All graph state lives in the site runtime database. Generic core code knows
entity semantics but no locality. Identity merges, relationships, public-money
records and public profiles are fail-closed unless explicit evidence is bound.
"""
...
ENTITY_TYPES = {
    "PERSON", "ARTIST", "ORGANIZATION", "COMPANY", "INSTITUTION", "EVENT",
    "VENUE", "PLACE", "PROJECT", "PUBLIC_MONEY_ITEM", "DOCUMENT", "STORY", "MEDIA_ASSET",
}
...
def slugify(value: str) -> str:
    result = normalize_alias(value).replace(" ", "-")
    _require(bool(result), "entity name cannot produce an empty slug")
    return result
...
def _provenance(provenance: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in provenance:
        ...
        _require(item["source_url"].startswith(("http://", "https://")), "provenance source_url must be HTTP(S)")
        ...
        values[key] = item
    _require(bool(values), "provenance is mandatory")
...
def upsert_entity(
    ...
    *,
    ...
    slug = slugify(name)
    chosen_id = _clean(entity_id) or _hash_id("ent", instance_id, kind, slug)
        ...
        if changed:
            try:
                ...
                _emit(
                    ...
                    aggregate_id=current["entity_id"],
                    event_type="KNOWLEDGE_ENTITY_UPSERTED",
                    reason="evidence-bound entity updated",
                    payload={"entity_type": kind, "fingerprint": fingerprint},
    ...
    try:
        ...
        _emit(
            ...
            instance_id=instance_id,
            aggregate_type="knowledge_entity",
            aggregate_id=chosen_id,
            event_type="KNOWLEDGE_ENTITY_UPSERTED",
            reason="evidence-bound entity created",
            payload={"entity_type": kind, "fingerprint": fingerprint},
            engine_version=engine_version,
            created_at=now,
        )
...
def record_edge(
    ...
    *,
    ...
    sources = _provenance(provenance)
    attrs = dict(attributes or {})
    edge_id = _hash_id("edge", instance_id, subject_entity_id, relation, object_entity_id, _clean(valid_from), _clean(valid_to))
    fingerprint = _stable_hash({
    ...
    if existing is not None:
        ...
        if changed:
            conn.execute(
                ...
                SET relation_basis=?,attributes_json=?,provenance_json=?,fingerprint=?,updated_at=?
            ...
            _emit(
                conn,
                instance_id=instance_id,
                aggregate_type="knowledge_edge",
                aggregate_id=edge_id,
                event_type="KNOWLEDGE_EDGE_UPSERTED",
                reason="documented relation evidence updated",
                payload={"subject": subject_entity_id, "predicate": relation, "object": object_entity_id},
                engine_version=engine_version,
                created_at=now,
            )
            conn.commit()
        row = conn.execute("SELECT * FROM knowledge_edges WHERE instance_id=? AND edge_id=?", (instance_id, edge_id)).fetchone()
        return _decode_edge(row), changed
    conn.execute(
        """
    ...
    _emit(
        ...
        aggregate_id=edge_id,
        event_type="KNOWLEDGE_EDGE_UPSERTED",
        reason="documented relation recorded",
        payload={"subject": subject_entity_id, "predicate": relation, "object": object_entity_id},
        engine_version=engine_version,
        created_at=now,
    )
...
def bind_published_story(
    ...
    *,
    ...
    _emit(
        ...
        event_type="KNOWLEDGE_STORY_LINKED",
        reason="published story enriched with evidence-backed entity",
        payload={"entity_id": entity_id, "role": relation_role},
        engine_version=engine_version,
        created_at=now,
    )
    conn.commit()
...
def _require_entity(conn: sqlite3.Connection, instance_id: str, entity_id: str | None) -> dict[str, Any] | None:
    return get_entity(conn, instance_id=instance_id, entity_id=entity_id) if entity_id else None
...
def record_public_money(
    ...
    *,
    ...
    for optional_id in (project_entity_id, event_entity_id, document_entity_id):
        _require_entity(conn, instance_id, optional_id)
    if story_id:
        _require(conn.execute("SELECT 1 FROM stories WHERE instance_id=? AND story_id=?", (instance_id, story_id)).fetchone() is not None, "public-money story is not part of the instance")
    ...
    _require(bool(CURRENCY_RE.fullmatch(code)), "currency must be a three-letter code")
    why = _clean(purpose)
    _require(bool(why), "public-money purpose is required")
    sources = _provenance(provenance)
    attrs = dict(attributes or {})
    money_item_id = _hash_id(
        "money", instance_id, payer_entity_id, beneficiary_entity_id, str(amount_minor), code, why,
        _clean(effective_date), _clean(project_entity_id), _clean(event_entity_id), _clean(document_entity_id),
    )
    fingerprint = _stable_hash({
    ...
    conn.execute(
        ...
        """,
        (
            instance_id, money_item_id, payer_entity_id, beneficiary_entity_id, amount_minor, code, why,
            project_entity_id, event_entity_id, document_entity_id, story_id, _clean(effective_date) or None,
            json.dumps(attrs, ensure_ascii=False, sort_keys=True),
            json.dumps(sources, ensure_ascii=False, sort_keys=True), fingerprint, now, now,
        ),
    )
    _emit(
        conn,
        instance_id=instance_id,
        aggregate_type="public_money",
        aggregate_id=money_item_id,
        event_type="PUBLIC_MONEY_RECORDED",
        reason="evidence-bound public-money item recorded",
        payload={"payer": payer_entity_id, "beneficiary": beneficiary_entity_id, "amount_minor": amount_minor, "currency": code},
        engine_version=engine_version,
        created_at=now,
    )
    conn.commit()
...
def record_timeline_event(
    ...
    *,
    ...
    if story_id:
        _require(conn.execute("SELECT 1 FROM stories WHERE instance_id=? AND story_id=?", (instance_id, story_id)).fetchone() is not None, "timeline story is not part of the instance")
    sources = _provenance(provenance)
    ...
    _emit(
        ...
        event_type="KNOWLEDGE_TIMELINE_RECORDED",
        reason="evidence-bound timeline item recorded",
        payload={"timeline_event_id": event_id, "event_date": date, "event_kind": kind},
        engine_version=engine_version,
        created_at=now,
    )
    conn.commit()
...
def project_public_profile(conn: sqlite3.Connection, *, instance_id: str, entity_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise KnowledgeGraphError("site publication schema is required for public profile projection") from exc
    return {
        "entity": entity,
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        person_updated, changed = upsert_entity(
            conn,
            instance_id="alpha-local",
            entity_type="PERSON",
            canonical_name="Alex Example",
            provenance=_source("person-update"),
            engine_version=engine,
            summary="Updated from new evidence.",
            attributes={"occupation": "Example role"},
            is_public=True,
        )
        assert changed and person_updated["summary"] == "Updated from new evidence."
        ...
        assert person_updated["entity_id"] != beta_person["entity_id"]
        assert add_alias(conn, instance_id="alpha-local", entity_id=person_updated["entity_id"], alias="A. Example", provenance=_source("alias"))
        assert resolve_alias(conn, instance_id="alpha-local", alias="A Example")["entity_id"] == person_updated["entity_id"]
        assert resolve_alias(conn, instance_id="beta-local", alias="A Example") is None
        try:
            add_alias(conn, instance_id="alpha-local", entity_id=artist["entity_id"], alias="A. Example", provenance=_source("ambiguous"))
        except KnowledgeGraphError:
            pass
        else:
        ...
        edge, edge_created = record_edge(
            conn,
            instance_id="alpha-local",
            subject_entity_id=person_updated["entity_id"],
        ...
        assert bind_published_story(
            ...
            instance_id="alpha-local",
            story_id="published-story",
            entity_id=person_updated["entity_id"],
            role="SUBJECT",
            provenance=_source("story-link"),
            engine_version=engine,
        )
        profile = project_public_profile(conn, instance_id="alpha-local", entity_id=person_updated["entity_id"])
        ...
        assert profile_path(person_updated).startswith("/profiles/person/")
