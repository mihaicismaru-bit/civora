...
def _payload(ready_components: set[str]) -> dict:
    ...
    for component in COMPONENTS:
        ...
        repository_capabilities[capability_id] = {
            ...
            "evidence": (
                [
                    f"contract:self-improvement-{component}",
                    f"instance:{INSTANCE_ID}",
                    f"main-commit:synthetic-{component}",
                ]
                if ready
                else [
                    f"instance:{INSTANCE_ID}",
    ...
    return {
        ...
        "persisted": {
            "main_head": MAIN,
            "decisions": [],
            "capabilities": [_capability(component) for component in COMPONENTS],
            "backlog": [_backlog(component) for component in sorted(INITIAL_PENDING)],
        },
        "repository": {
            "main_head": MAIN,
...
def main() -> int:
    # Metrics and feedback activity alone must not imply a closed autonomous loop.
    ...
    assert partial_rows["controlled_change"]["status"] == "ACTIVE_UNIMPLEMENTED"
    assert partial_rows["validation"]["status"] == "ACTIVE_UNIMPLEMENTED"
    assert partial_rows["rollback"]["status"] == "ACTIVE_UNIMPLEMENTED"

    # Ranking influence may become independently ready without granting change authority.
    ...
    assert _active_backlog_components(ranked_result) == {
        "controlled_change",
        "validation",
        "rollback",
    }

    # A controlled-change component without validation/rollback still cannot close those safeguards.
    ...
    print(json.dumps({
        ...
        "metrics_feedback_do_not_close_loop": True,
        "ranking_influence_independent": True,
        "controlled_change_does_not_bypass_validation_or_rollback": True,
