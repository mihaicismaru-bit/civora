#!/usr/bin/env python3
"""PRS-046 acceptance: profile asset READY without remote readback stays UNCONFIRMED."""
from __future__ import annotations

import json
from copy import deepcopy

from reconciliation_engine import REQUIRED_FRESH_GATES, reconcile

CAP = "C-PROFILE-ASSET"


def main() -> int:
    payload = {
        "schema_version": 1,
        "scope_id": "synthetic-product",
        "instance_id": "synthetic-instance",
        "persisted": {
            "decisions": [],
            "capabilities": [{
                "capability_id": CAP,
                "desired_state": "EXTERNAL_LIVE",
                "code_state": "READY",
                "runtime_state": "DIRECT_NATIVE",
                "external_state": "LIVE_CONFIRMED",
                "direct_or_outbox": "DIRECT",
                "priority": "P0",
            }],
            "backlog": [],
        },
        "repository": {
            "scope_classification": "NO_SCOPE_CHANGE",
            "decisions": {},
            "capabilities": {CAP: {
                "code_state": "READY",
                "runtime_state": "DIRECT_NATIVE",
                "evidence": ["profile_asset:ready", "profile_configuration:ready"],
            }},
        },
        "external": {
            "decisions": {},
            "capabilities": {CAP: {
                "external_state": "UNCONFIRMED",
                "evidence": ["remote_readback:absent"],
            }},
        },
        "health_gates": {gate: True for gate in REQUIRED_FRESH_GATES},
    }
    before = deepcopy(payload)
    result = reconcile(payload)
    assert payload == before

    row = next(x for x in result["capabilities"] if x["capability_id"] == CAP)
    assert row["code_state"] == "READY"
    assert row["runtime_state"] == "DIRECT_NATIVE"
    assert row["external_state"] == "UNCONFIRMED"
    assert row["status"] == "PARTIAL"
    assert row["direct_or_outbox"] == "DIRECT"
    assert row["gap"] == "EXTERNAL_CONFIRMATION_GAP"
    external_live = row["status"] == "IMPLEMENTED" and row["external_state"] in {
        "CONFIRMED", "LIVE_CONFIRMED", "REMOTE_PUBLICATION_EVIDENCE_PRESENT"
    }
    assert external_live is False
    assert any(
        x.get("kind") == "FALSE_POSITIVE_PERSISTENCE"
        and x.get("capability_id") == CAP
        and x.get("field") == "external_state"
        for x in result["diagnostics"]
    )
    assert result["persistence_health"]["state"] == "RECONCILIATION_REQUIRED"

    print(json.dumps({
        "status": "PASS",
        "prs": "PRS-046",
        "capability_status": row["status"],
        "external_state": row["external_state"],
        "external_live": external_live,
        "gap": row["gap"],
        "repository_input_unchanged": True,
    }, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
