...
def _scheduler_publication(publication: dict[str, Any]) -> dict[str, Any]:
    """Drop catalog-only sealing fields exactly as scheduler enumeration does."""
    return {
        key: _clone(publication.get(key))
        for key in scheduler.DESCRIPTOR_FIELDS
...
def _reconstruct_sealed_job(
    ...
    scheduler_publication = _scheduler_publication(publication)
    job: dict[str, Any] = {
        "publication": scheduler_publication,
        "publication_id": publication_id,
