#!/usr/bin/env python3
"""Acceptance tests for Threads as an independent durable-outbox publication.

Threads publishing credentials or a verified adapter are deliberately not assumed.
The target is a complete native thread product with its own cadence, series, state,
...
class ThreadsOutboxAcceptance(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_channel_is_independent_outbox_only_publication(self) -> None:
        ...
        self.assertEqual("outbox_only", self.threads["status"])
        self.assertEqual(["thread", "text", "single_photo"], self.threads["native_formats"])
        self.assertEqual("none:threads-publishing-access-not-verified", self.threads["credentials_ref"])
        ...
        self.assertEqual([], self.threads["metrics"]["sources"])
    ...
    def test_registry_never_invents_unverified_threads_adapter_or_credentials(self) -> None:
