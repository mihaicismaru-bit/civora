...
def self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="civora-scope-") as td:
        ...
        (root / "valcea-clar/editorial").mkdir(parents=True)
        ...
        (root / "valcea-clar/editorial/fact_kernel_registry.json").write_text('{"generated_at":"t1"}\n', encoding="utf-8")
        ...
        config = ScopeConfig(
            ...
            exclude=(),
            refresh_only=("valcea-clar/**/*_state.json", "valcea-clar/editorial/fact_kernel_registry.json"),
        )
        ...
        (root / "valcea-clar/editorial/fact_kernel_registry.json").write_text('{"generated_at":"t2"}\n', encoding="utf-8")
        _run_git(["add", "."], cwd=root)
