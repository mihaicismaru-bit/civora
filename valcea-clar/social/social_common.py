#!/usr/bin/env python3
"""Shared fail-closed helpers for the VÂLCEA CLAR social publishing adapters."""
...
def platform_config(item: dict[str, Any], platform: str) -> dict[str, Any] | None:
    ...
    if not isinstance(platforms, dict):
        # Backward compatibility: legacy outbox entries remain Facebook-only.
...
def direct_photo_url(item: dict[str, Any], platform: str) -> str:
    ...
    candidates = [
        ...
        (item.get("image") or {}).get("direct_source_url")
...
def canonical_photo_url(item: dict[str, Any]) -> str:
    ...
    if explicit:
        ...
        if parsed.scheme != "https" or parsed.hostname not in CANONICAL_HOSTS:
            raise ValueError(
                f"TikTok photo URL must be on valceaclar.ro for verified ownership: "
                f"{explicit}"
            )
        return explicit
