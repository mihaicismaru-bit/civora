...
Direct Post remains blocked until the TikTok app is approved for public posting,
the account token is present, current creator settings are queried, and explicit
per-post consent is recorded by the valceaclar.ro administration surface.
"""
...
def remote_photo_preflight(url: str) -> None:
    ...
    if "image" not in content_type and not (
        ...
        raise RuntimeError(
            f"TikTok media URL returned unsupported content: {content_type or 'unknown'}"
        )
...
def consent_record(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if value.get("source") not in ALLOWED_CONSENT_SOURCES:
        raise ValueError(
            f"{item.get('id')} consent must come from valceaclar.ro administration"
        )
