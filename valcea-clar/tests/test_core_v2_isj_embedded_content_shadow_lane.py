...
class ISJEmbeddedContentShadowLaneTests(unittest.TestCase):
    ...
    def test_verified_pdf_bytes_are_captured_but_not_promoted(self):
        pdf = b"%PDF-1.7\nshadow-test\n%%EOF\n"
        ...
        row = result["rows"][0]
        self.assertEqual(row["state"], "DOCUMENT_CONTENT_CAPTURED_SHADOW")
    ...
    def test_non_live_does_not_fetch(self):
        called = False
        def fetcher(url):
            nonlocal called
            called = True
    ...
    def test_non_current_binding_is_not_selected(self):
        ...
        result = capture_verified_document_contents(report, allow_network=True, fetcher=lambda url: (_ for _ in ()).throw(AssertionError()))
