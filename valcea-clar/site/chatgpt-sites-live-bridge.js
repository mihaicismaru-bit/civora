(() => {
  ...
  const safePath = (v) => {
    ...
    return /^\/stiri\/[a-z0-9-]+\/$/.test(value) ? value : '/';
  };
  const storyHref = (story) => safePath(story?.path);
  ...
  function styleOnce() {
    ...
    style.textContent = `
      ...
      .vc-article{max-width:850px;margin:0 auto;padding:34px 0 24px}.vc-article h1{font:800 clamp(38px,6vw,64px)/1.05 Georgia,serif;letter-spacing:-.025em;margin:8px 0 15px}.vc-article .vc-dek{font-size:21px}.vc-body{font:18px/1.72 Georgia,serif;margin-top:27px}.vc-body p{margin:0 0 20px}.vc-back{display:inline-block;margin-top:30px;font-weight:800;color:var(--vc-navy)!important;text-decoration:none}.vc-back:hover{text-decoration:underline}
  ...
  function shell(content) {
    return `<div id="vc-runtime">
      <header class="vc-top"><div class="vc-mast"><div><a href="/" class="vc-storylink"><div class="vc-brand"><span>VÂLCEA CLAR</span></div></a><div class="vc-tag">Știrile Vâlcii, fără zgomot.</div></div><div>valceaclar.ro</div></div><nav class="vc-nav"><a href="/">Acasă</a><a href="/#stiri">Știri locale</a><a href="/unde-iesim/">Unde ieșim</a></nav></header>${content}</div>`;
  }
  ...
  function renderHome(feed) {
    ...
    const content = `<main class="vc-main"><div class="vc-livebar"><span class="vc-pill">ACTUALIZAT LIVE</span><span class="vc-time">${esc(updated)}</span></div>
      <div class="vc-grid"><section><article class="vc-hero"><div class="vc-kicker">${esc(lead.section)}</div><h1><a class="vc-storylink" href="${esc(storyHref(lead))}">${esc(lead.headline)}</a></h1><p class="vc-dek">${esc(lead.dek)}</p>${paragraphs}<div class="vc-sources">${sourceLinks(lead)}</div></article><h2 class="vc-section-title" id="stiri">Cele mai noi știri verificate</h2><div class="vc-cards">${cards}</div></section>
      <aside class="vc-side"><h2>Top știri</h2>${side}<section class="vc-venues"><h2>Unde ieșim</h2>${venues}</section></aside></div>
      <div class="vc-note">VÂLCEA CLAR publică fiecare material imediat ce trece verificarea editorială. Edițiile de dimineață și seară sunt doar recapuri și nu întârzie știrile.</div><div class="vc-status">Redacție live · story-first · fără API LLM plătit · ${stories.length} materiale publicabile</div></main>`;
    root.innerHTML = shell(content);
    document.title = 'VÂLCEA CLAR — Știri live din Vâlcea';
  }

  function storyFromPath(feed) {
    const path = window.location.pathname.replace(/\/{2,}/g, '/');
    if (!/^\/stiri\/[a-z0-9-]+\/$/.test(path)) return null;
    return (feed.stories || []).find(story => story.path === path) || false;
  }
  ...
  function renderStory(feed, story) {
    ...
    const content = `<main class="vc-main"><article class="vc-article"><div class="vc-kicker">${esc(story.section)}</div><h1>${esc(story.headline)}</h1><p class="vc-dek">${esc(story.dek)}</p><div class="vc-status">Actualizat ${esc(feed.generated_at || '')}</div><div class="vc-body">${body}</div><div class="vc-sources">Surse: ${sourceLinks(story)}</div><a class="vc-back" href="/">← Înapoi la VÂLCEA CLAR</a></article></main>`;
    root.innerHTML = shell(content);
    document.title = `${story.headline} — VÂLCEA CLAR`;
  }

  function render(feed) {
    if (feed.publication_model !== 'continuous_story_first' || !Array.isArray(feed.stories)) {
      throw new Error('Feed is not story-first');
    }
    const routedStory = storyFromPath(feed);
    if (routedStory === null) renderHome(feed);
    else renderStory(feed, routedStory);
    lastFingerprint = `${feed.generated_at || ''}:${(feed.stories || []).map(s => `${s.id}:${s.headline}:${(s.paragraphs || []).length}`).join('|')}`;
  }
  ...
  async function refresh() {
    try {
      ...
      const fingerprint = `${feed.generated_at || ''}:${(feed.stories || []).map(s => `${s.id}:${s.headline}:${(s.paragraphs || []).length}`).join('|')}`;
      if (!lastFingerprint || fingerprint !== lastFingerprint) render(feed);
    } catch (err) {
