...
Only allow-listed first-party ISJ targets are fetched. External documents remain
pure discovery targets. A successful read proves URL identity and observed bytes,
not admission capacity, results, vacancies, deadlines, eligibility or publishable
facts.
"""
...
from isj_valcea_education_reference_adapter import (
    ...
    build_live_receipt as build_index_receipt,
)
...
MAX_DETAILS = 12
MAX_BYTES = 3_000_000
ALLOWED_CONTENT_TYPES = {"text/html", "text/plain", "application/pdf", "application/octet-stream"}
...
class VisibleTextParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if name in {"script", "style", "noscript", "svg"}:
            self.skip += 1
        elif name == "title" and not self.skip:
            self.in_title = True
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if name == "title":
            self.in_title = False
        elif name in {"script", "style", "noscript", "svg"} and self.skip:
            self.skip -= 1
...
def _extract_html_evidence(body: bytes, index_title: str) -> tuple[str | None, str | None, tuple[str, ...]]:
    ...
    explicit_date = match.group(0) if match else None
    words = re.findall(r"[A-Za-zĂÂÎȘȚăâîșț0-9-]{5,}", index_title)[:8]
    lowered = visible.casefold()
    fragments: list[str] = []
    for word in words:
        pos = lowered.find(word.casefold())
        if pos < 0:
            continue
        fragment = " ".join(visible[max(0, pos - 120): min(len(visible), pos + len(word) + 220)].split())[:420]
...
def build_live_receipt() -> dict[str, Any]:
    index = build_index_receipt(require_reference=True)
    candidates = [
    ...
    for ref in candidates:
        try:
            body, final_url, content_type = _fetch_detail(str(ref["target_url"]))
            visible_title = explicit_date = None
            fragments: tuple[str, ...] = ()
            ...
            details.append(DetailEvidence(
                topic_class=str(ref["topic_class"]), index_title=str(ref["title"]), detail_url=final_url,
                detail_host=(urlsplit(final_url).hostname or "").lower(), content_type=content_type,
                content_length=len(body), detail_sha256=_sha256(body),
                index_evidence_sha256=str(ref["evidence_sha256"]), visible_title=visible_title,
                explicit_date_text=explicit_date, evidence_fragments=fragments, verification_state=state,
            ))
    ...
    payload: dict[str, Any] = {
        "schema": SCHEMA, "status": "PASS", "source_family": SOURCE_FAMILY,
        "authority_class": AUTHORITY_CLASS, "observation_state": OBSERVATION_STATE,
        "fetched_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "coverage_note": "BOUNDED_HIGH_VALUE_FIRST_PARTY_DETAIL_VERIFICATION_NOT_EXHAUSTIVE",
        "index_run_id": index.get("run_id"), "index_reference_count": index.get("reference_count", 0),
        "eligible_first_party_detail_count": len(candidates), "detail_evidence_count": len(details),
        "detail_hold_count": len(holds), "details": [asdict(item) for item in details], "holds": holds,
        "interpretation": "FIRST_PARTY_DETAIL_BYTES_AND_EXPLICIT_TEXT_ARE_EVIDENCE_CONTEXT_ONLY;MATERIAL_FIELDS_REQUIRE_SEPARATE_FIELD_LEVEL_VERIFICATION",
        **NON_AUTHORIZING_FLAGS,
    }
...
def _self_test() -> None:
    ...
    html = ("<html><head><title>Admitere 2026 - ISJ Valcea</title></head><body>"
            "<script>Rezultate false 01.01.2099</script>Admitere liceu - publicat 02.09.2026. "
            "Locuri libere pentru etapa urmatoare.</body></html>").encode()
    title, date_text, fragments = _extract_html_evidence(html, "Locuri libere admitere liceu 2026")
    assert title == "Admitere 2026 - ISJ Valcea" and date_text == "02.09.2026" and fragments
    assert all(value is False for value in NON_AUTHORIZING_FLAGS.values())
    assert build_index_receipt is not build_live_receipt
...
def main() -> int:
    ...
    try:
        ...
        print(f"HOLD_SOURCE_FETCH_FAILED:{type(exc).__name__}:{exc}", file=sys.stderr); return 2
