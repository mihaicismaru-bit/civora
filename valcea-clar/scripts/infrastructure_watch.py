...
def fetch(url: str, terms: list[str], timeout: int = 20) -> dict[str, Any]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as response:
            ...
            return {"reachable": True, "http_status": int(response.status), "final_url": str(response.geturl()), "fingerprint": fp, "match_count": hits, "error": None}
...
def build(live: bool, previous: dict[str, Any]) -> dict[str, Any]:
    ...
    for binding in monitor.get("source_bindings") or []:
        ...
        base = {"id": sid, "publisher": binding.get("publisher"), "url": binding.get("url"), "tier": binding.get("tier")}
    ...
    return {
        ...
        "sources": sources,
        "subprojects": [{"id": lead.get("id"), "label": lead.get("label"), "verification_status": lead.get("verification_status"), "public_projection": False} for lead in leads],
        "publication_contract": {"monitor_is_not_story": True, "source_change_is_not_material_fact": True, "public_projection": False, "normal_story_ready_gate_required": True},
    }
...
def validate_registry() -> None:
    ...
    assert ids == {"a1-section-2-boita-cornetu", "a1-section-3-cornetu-tigveni", "a1-section-4-tigveni-curtea-de-arges", "dn73c-tigveni-ramnicu-valcea", "ganesa-ramnicu-valcea-tigveni-corridor"}
