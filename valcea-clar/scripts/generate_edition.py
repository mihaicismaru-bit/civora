...
The generator is deliberately deterministic and fail-closed. It only renders
structured facts that have already passed an editorial evidence gate. It never
invents missing facts and it can publish a shorter edition when the verified
input pool is small.
"""
...
def operational_blocks() -> list[dict]:
    ...
    if meta:
        ...
        monitored = int(summary.get("ingest_records") or summary.get("ingest_records_total") or 0)
        if not monitored:
            # Current reconciler exposes ingest_records at the top level in some versions.
            monitored = int(recon.get("ingest_records") or 0)
...
def render_markdown(now: datetime, slot: str, items: list[dict], status_note: str) -> str:
    ...
    title = f"# VÂLCEA CLAR — Ediția de {label}\n\n"
    stamp = now.strftime("%d.%m.%Y · %H:%M")
    lines = [title, f"**Actualizată automat la {stamp} · Europe/Bucharest**\n\n"]
    if status_note:
...
def write_outputs(now: datetime, slot: str, facts: list[dict]) -> tuple[Path, Path, dict]:
    ...
    # Avoid publishing an empty shell: at least one verified editorial fact must exist.
    ...
    pointer = {
        ...
        "homepage_role": "primary_lead" if publish else "hidden",
    }
    (SITE / "current_edition.json").write_text(json.dumps(pointer, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
