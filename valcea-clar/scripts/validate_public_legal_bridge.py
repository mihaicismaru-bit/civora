...
def main() -> int:
    ...
    for slug, page in doc["pages"].items():
        ...
        assert "server-rendered" in text or "<main" in text
    ...
    assert not leaked, f"Secret marker leaked into public legal artifacts: {leaked}"

    print(
        "VÂLCEA CLAR public legal routes: PASS "
        "(canonical navigation v2 + server-rendered/static repository contract; remote HTTP remains separate)"
    )
    return 0
