#!/usr/bin/env python3
"""Fail closed if a retired alternate publication surface is reintroduced.
...
and reader assets so retired generators, bridges, build targets, or deployment
references cannot silently return.
"""
...
MARKERS = (
    ...
    "build_" + "chat" + "gpt_sites_overlay.py",
    "overlay_runtime_export.py",
    "public_ux_export_contract.py",
    "public_ux_overlay.py",
    "chat" + "gpt-sites-live-bridge",
    "chat" + "gpt-sites-route-bridge",
)
...
def main() -> int:
    ...
    for path in candidates():
        ...
        if path_markers:
            hits.append(f"{rel}: retired path marker")
        try:
        ...
        if content_markers:
            hits.append(f"{rel}: retired content/process marker")

    if hits:
        print("Retired publication surface detected:")
        for hit in hits:
            print(f"- {hit}")
        return 1

    print("VÂLCEA CLAR retired alternate publication surface: ABSENT")
    return 0
