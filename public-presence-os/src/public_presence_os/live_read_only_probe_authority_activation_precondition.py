...
def _validate_policy(policy: dict[str, Any]) -> None:
    ...
    if policy.get("parent_authorization_receipt_checkpoint") != CP68_CHECKPOINT or policy.get("parent_control_checkpoint") != PARENT_CONTROL_CHECKPOINT:
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_PARENT_DRIFT")
    if tuple(policy.get("active_platforms", ())) != EXPECTED_ACTIVE or tuple(policy.get("matrix_rows", ())) != MATRIX_ROWS:
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_SCOPE_OR_MATRIX_DRIFT")
    ...
    required_true = (
        "local_only", "zero_io_required", "canonical_json_required", "sha256_binding_required",
        "injected_evaluation_time_required", "synthetic_fixture_allowed_for_validation_only",
        "platform_subset_of_active_lanes_required", "receipt_validity_window_must_contain_evaluation_time",
        "exact_cp68_contract_binding_required", "exact_receipt_hash_binding_required", "exact_cp67_parent_binding_required",
        ...
        "keychain_read_forbidden", "oauth_forbidden", "real_account_lookup_forbidden", "account_connection_forbidden",
    ...
    if evaluation.get("receipt_decision_must_equal") != "GRANT" or evaluation.get("scope_must_equal") != REQUESTED_SCOPE:
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_DECISION_OR_SCOPE_POLICY")
...
def _row(name: str, satisfied: bool) -> ActivationPreconditionRow:
    return ActivationPreconditionRow(name=name, satisfied=satisfied, evidence_sha256=_hash({"name": name, "satisfied": satisfied}))
...
def _build_cp68_fixture(root: Path, cp68_contract: Any) -> tuple[Any, Any, Any]:
    cp67 = compile_live_read_only_probe_authorized_session_request(root, load_json(root / "config" / "live_read_only_probe_authorized_session_request_policy.json"))
    ...
    submission = {
        ...
        "human_reference_sha256": sha256(b"cp68-human").hexdigest(),
        "evidence_sha256": sha256(b"cp68-evidence").hexdigest(),
        "nonce": "cp68-synthetic-nonce-0001",
    }
    receipt = compile_immutable_session_authorization_receipt(cp67, packet, handoff, submission, synthetic_fixture=True)
    dry_run = build_activation_dry_run(receipt, cp67)
    if (receipt.receipt_id, receipt.receipt_hash, dry_run.dry_run_id, dry_run.dry_run_hash) != (
        cp68_contract.receipt_id, cp68_contract.receipt_hash, cp68_contract.dry_run_id, cp68_contract.dry_run_hash
    ):
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_CP68_RECEIPT_EXACT_BINDING")
    return cp67, receipt, dry_run
...
def evaluate_authority_activation_preconditions(
    ...
    *,
    ...
    validate_immutable_session_authorization_receipt(receipt, cp67_contract)
    ...
    required_snapshot = {
        ...
        "control_plane_promoted", "environment_reads", "keychain_reads", "oauth_attempts", "real_account_lookups",
        "network_attempts", "live_probe_attempts", "publish_attempts", "external_writes", "deploy_attempts", "paid_service_uses",
    }
    if set(runtime_snapshot) != required_snapshot:
    ...
    cp68_contract_bound = (
        (receipt.cp67_contract_id, receipt.cp67_contract_hash) == (cp68_contract.cp67_contract_id, cp68_contract.cp67_contract_hash)
        and (cp67_contract.contract_id, cp67_contract.contract_hash) == (cp68_contract.cp67_contract_id, cp68_contract.cp67_contract_hash)
    )
    receipt_bound = (receipt.receipt_id, receipt.receipt_hash, dry_run.dry_run_id, dry_run.dry_run_hash) == (
        cp68_contract.receipt_id, cp68_contract.receipt_hash, cp68_contract.dry_run_id, cp68_contract.dry_run_hash
    )
    active_subset = (
    ...
    zero_io = all(runtime_snapshot[key] == 0 for key in counter_keys)
    facts = {
        "CP68_CONTRACT_EXACT_BOUND": cp68_contract_bound,
        "CP68_RECEIPT_EXACT_BOUND": receipt_bound,
    ...
    rows = tuple(_row(name, facts[name]) for name in MATRIX_ROWS)
...
def validate_authority_activation_precondition_matrix(matrix: AuthorityActivationPreconditionMatrix, cp68_contract: Any, receipt: Any, dry_run: Any) -> None:
    ...
    if (matrix.receipt_id, matrix.receipt_hash, matrix.dry_run_id, matrix.dry_run_hash) != (receipt.receipt_id, receipt.receipt_hash, dry_run.dry_run_id, dry_run.dry_run_hash):
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_MATRIX_RECEIPT_BINDING")
    if tuple(row.name for row in matrix.rows) != MATRIX_ROWS or matrix.all_structural_preconditions_satisfied != all(row.satisfied for row in matrix.rows):
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_MATRIX_ROWS")
    if any(getattr(matrix, key) is not False for key in (
        "runtime_authorization_effective", "authority_activated", "network_allowed", "live_probe_allowed",
        "account_connection_allowed", "publish_allowed", "external_write_allowed", "deploy_allowed", "control_plane_promoted",
    )):
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_MATRIX_AUTHORITY")
    if matrix.global_kill_switch_engaged is not True or not _hex(matrix.matrix_hash):
        raise LiveReadOnlyProbeAuthorityActivationPreconditionHold("HOLD_CP69_MATRIX_GUARD")
...
def evaluate_from_root(root: Path, policy: dict[str, Any], *, evaluated_at_utc: str | None = None) -> AuthorityActivationPreconditionMatrix:
    root = root.resolve()
    _validate_policy(policy)
    _validate_root(root)
    cp68 = compile_live_read_only_probe_session_authorization_receipt(root, load_json(root / "config" / "live_read_only_probe_session_authorization_receipt_policy.json"))
    cp67, receipt, dry_run = _build_cp68_fixture(root, cp68)
    runtime = load_json(root / "config" / "runtime_policy.json")
    return evaluate_authority_activation_preconditions(
        cp68, cp67, receipt, dry_run,
        evaluated_at_utc=evaluated_at_utc or policy["synthetic_evaluation_utc"],
        runtime_snapshot=_zero_runtime_snapshot(runtime),
    )


def compile_live_read_only_probe_authority_activation_precondition_matrix(root: Path, policy: dict[str, Any]) -> LiveReadOnlyProbeAuthorityActivationPreconditionContract:
    root = root.resolve()
    _validate_policy(policy)
    _validate_root(root)
    cp68 = compile_live_read_only_probe_session_authorization_receipt(root, load_json(root / "config" / "live_read_only_probe_session_authorization_receipt_policy.json"))
    ...
    contract = LiveReadOnlyProbeAuthorityActivationPreconditionContract(contract_id=f"cp69_contract_{digest[:24]}", contract_hash=digest, **body)
    validate_live_read_only_probe_authority_activation_precondition_contract(contract)
    return contract


def validate_live_read_only_probe_authority_activation_precondition_contract(contract: LiveReadOnlyProbeAuthorityActivationPreconditionContract) -> None:
    if (
        contract.model_version, contract.engine_version, contract.checkpoint, contract.parent_control_checkpoint,
        contract.parent_authorization_receipt_checkpoint, contract.parent_cp68_state, contract.state, contract.next_unit,
    ) != (MODEL_VERSION, ENGINE_VERSION, CHECKPOINT, PARENT_CONTROL_CHECKPOINT, CP68_CHECKPOINT, CP68_STATE, STATE, NEXT_UNIT):
    ...
    for key in (
        ...
        "network_allowed", "network_attempted", "live_probe_allowed", "live_probe_attempted", "publish_allowed",
        "publish_attempted", "external_write_allowed", "external_write_performed", "control_plane_promoted", "deploy_allowed",
    ...
    if not all(_hex(value) for value in (
        contract.contract_hash, contract.cp68_contract_hash, contract.matrix_hash, contract.policy_sha256,
        contract.cp68_policy_sha256, contract.runtime_policy_sha256, contract.module_registry_sha256,
    )):
