...
def test_registry_unique_ids():
    ...
    assert any(m["id"]=="M26_META_OFFLINE_EVIDENCE_VALIDATOR" and m["status"]=="CP57_SYNTHETIC_EVIDENCE_BUNDLE_VALIDATOR_DRY_RUN_LOCAL_ONLY" for m in r["modules"])
    assert any(m["id"]=="M27_META_PILOT_READINESS" and m["status"]=="CP58_OFFLINE_READINESS_AGGREGATED_LIVE_CONNECTION_HOLD" for m in r["modules"])
...
def test_reimplementation_priority_closes_executable_source_backlog():
    ...
    assert states["M26_META_OFFLINE_EVIDENCE_VALIDATOR"]=="CP57_SYNTHETIC_EVIDENCE_BUNDLE_VALIDATOR_DRY_RUN_LOCAL_ONLY"
    assert states["M27_META_PILOT_READINESS"]=="CP58_OFFLINE_READINESS_AGGREGATED_LIVE_CONNECTION_HOLD"
    assert p["next"]=="CP59_PILOT_PACKAGE_COMPLETENESS_MANIFEST_AND_FINAL_OFFLINE_ACCEPTANCE_SUITE"
