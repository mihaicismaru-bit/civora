...
def test_m01_is_now_executable_but_remaining_historical_modules_hold():
    r=run_synthetic_rehearsal(ROOT)
    pipeline={s.module_id:s for s in r.stages}
    assert pipeline["M01_RADAR"].state=="PASS_EXECUTABLE_SOURCE"
    assert "M01_RADAR:EXECUTABLE_SOURCE_UNAVAILABLE" not in r.blockers
    for module_id in REQUIRED_PIPELINE[1:]:
