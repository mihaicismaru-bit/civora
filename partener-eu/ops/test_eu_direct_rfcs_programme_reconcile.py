...
import eu_direct_rfcs_programme_intelligence as source_mod
import eu_direct_rfcs_programme_reconcile as rec_mod
...
class RFCSReconcileTests(unittest.TestCase):
    ...
    def test_semantic_change_becomes_pipeline_watch_only(self):
        prev = snapshot("prev", "2026-09-03T09:00:00+00:00")
        cur = snapshot("cur", "2026-09-03T10:00:00+00:00", " changed official programme wording")
        rec = rec_mod.reconcile(cur, prev)
        self.assertEqual(rec["reconciliation_state"], "RFCS_PROGRAMME_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING")
    ...
    def test_identity_drift_rejected(self):
        prev = snapshot("prev", "2026-09-03T09:00:00+00:00")
        cur = snapshot("cur", "2026-09-03T10:00:00+00:00")
        prev["evidence"][0]["authority_url"] = "https://rea.ec.europa.eu/drift"
    ...
    def test_romania_fit_drift_rejected_as_identity_boundary(self):
        prev = snapshot("prev", "2026-09-03T09:00:00+00:00")
        cur = snapshot("cur", "2026-09-03T10:00:00+00:00")
        prev["romania_programme_fit"] = "ELIGIBLE"
        with self.assertRaises(ValueError):
            rec_mod.reconcile(cur, prev)
