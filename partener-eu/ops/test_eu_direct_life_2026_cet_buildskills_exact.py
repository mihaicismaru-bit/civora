...
from eu_direct_life_2026_cet_buildskills_reconcile import reconcile, validate_reconciliation

PROGRAMME_CODE = "LIFE-SYNTHETIC-2021-2027"
STATUS_OPEN = "31094501"
STATUS_CLOSED = "31094503"
CALL_IDENTIFIER = "LIFE-2026-CET"


def facet_payload(status_code=STATUS_OPEN, status_label="Open", programme_label="Programme for the Environment and Climate Action (LIFE)"):
    return {
        "facets": [
...
def search_payload(status_code=STATUS_OPEN, deadline="2026-09-16T17:00:00+02:00"):
    return [
        {
            "identifier": TOPIC_IDENTIFIER,
            "topicAbbreviation": TOPIC_IDENTIFIER,
            "callIdentifier": CALL_IDENTIFIER,
            "type": "1",
            "frameworkProgramme": PROGRAMME_CODE,
            "programmePeriod": "2021 - 2027",
            "status": status_code,
            "title": "BUILD UP Skills - National Platforms on energy efficiency skills for the clean energy transition",
...
def cinea_html(status="Open", deadline="16 September 2026, 17:00 (CEST)"):
    return f"""<!doctype html><html><body>
    <h1>BUILD UP Skills - National Platforms on energy efficiency skills for the clean energy transition</h1>
    <div>Call for proposals</div>
    <div>Status {status}</div>
    <div>Reference {TOPIC_IDENTIFIER}</div>
    <div>Deadline date {deadline}</div>
    <div>Funding programme Programme for the Environment and Climate Action (LIFE) (2021/2027)</div>
...
def main():
    baseline = collect_exact(
        ...
        fetched_at="2026-09-08T05:00:00+00:00",
        post_func=make_post(),
    ...
    assert baseline["structured_snapshot"]["topic_identifier"] == TOPIC_IDENTIFIER
    assert baseline["structured_snapshot"]["call_identifier"] == CALL_IDENTIFIER
    assert "life" in baseline["structured_snapshot"]["programme_label"].casefold()
    assert baseline["deadline_candidate"] == "16 September 2026, 17:00 (CEST)"
    ...
    replay = collect_exact(
        ...
        fetched_at="2026-09-08T05:00:02+00:00",
        post_func=make_post(),
        cinea_fetcher=make_cinea("Open"),
    )
    ...
    closed = collect_exact(
        ...
        fetched_at="2026-09-08T05:00:03+00:00",
    ...
    degraded = collect_exact(
        ...
        fetched_at="2026-09-08T05:00:04+00:00",
        post_func=make_post(),
    ...
    conflict = collect_exact(
        ...
        fetched_at="2026-09-08T05:00:05+00:00",
        post_func=make_post(search=search_payload(STATUS_CLOSED), facet=facet_payload(STATUS_CLOSED, "Closed")),
        cinea_fetcher=make_cinea("Open"),
    )
    ...
    bad_programme = facet_payload(programme_label="Digital Europe Programme")
    degraded_programme = collect_exact(
        run_id="bad-programme",
        fetched_at="2026-09-08T05:00:06+00:00",
    ...
    print("eu_direct_life_2026_cet_buildskills exact + reconcile regression: PASS")
