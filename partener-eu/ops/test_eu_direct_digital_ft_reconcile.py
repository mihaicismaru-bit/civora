...
def evidence(ts="2026-09-01T20:10:00+00:00", state="OPEN_CALL", status="Open", deadline="2027-03-03T17:00:00Z"):
...
def main():
    ...
    same = evidence("2026-09-01T20:20:00+00:00")
    no_change = reconcile(same, current)
    assert no_change["reconciliation_state"] == "NO_CHANGE"
    assert no_change["semantic_change_count"] == 0

    closed = evidence("2026-09-01T20:30:00+00:00", state="CLOSED_CALL", status="Closed")
    changed = reconcile(closed, same)
    assert changed["reconciliation_state"] == "DIGITAL_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
    ...
    print("eu_direct_digital_ft_reconcile regression: PASS")
