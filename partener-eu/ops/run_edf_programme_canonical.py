...
SNAPSHOT_SCHEMA = "PARTENER_EU_EDF_PROGRAMME_INTELLIGENCE_V1"
RECONCILIATION_SCHEMA = "PARTENER_EU_EDF_PROGRAMME_RECONCILIATION_V1"
PROGRAMME_ID = "EDF"
ARTIFACT_PREFIXES = (
    "partener-eu-eu-direct-programme-intelligence-",
    "partener-eu-edf-programme-history-",
    "partener-eu-edf-programme-proof-",
)
...
def candidate_priority(path: pathlib.Path) -> tuple[int, str]:
    ...
    if "/history/" in text or text.endswith("/history/edf-programme-intelligence.json"):
        return (0, text)
    if "/current/" in text:
        return (1, text)
    return (2, text)
...
def restore_previous(root: pathlib.Path, registry: dict[str, Any]) -> dict[str, Any]:
    ...
    scratch = pathlib.Path("/tmp/partener-eu-edf-canonical-history-scan")
    ...
    for _, artifact_id, artifact_name in rows:
        ...
        candidates = sorted(unpack.rglob("edf-programme-intelligence.json"), key=candidate_priority)
        for candidate_path in candidates:
            ...
            if inventory != expected:
                continue
            dump(previous_dir / "edf-programme-intelligence.json", candidate)
            metadata.update({
                "previous_found": True,
...
def enforce_boundary(root: pathlib.Path) -> dict[str, Any]:
    current = load(root / "current" / "edf-programme-intelligence.json")
    reconciliation = load(root / "current" / "edf-programme-reconciliation.json")
    if current.get("schema") != SNAPSHOT_SCHEMA or current.get("source_family") != "EU_DIRECT" or current.get("programme_id") != PROGRAMME_ID:
        raise SystemExit("FAIL EDF canonical snapshot identity drift")
    if current.get("programme_family") != "European Defence Fund":
        raise SystemExit("FAIL EDF canonical programme family drift")
    if current.get("source_count") != 3 or int(current.get("healthy_source_count") or 0) + int(current.get("degraded_source_count") or 0) != 3:
        raise SystemExit("FAIL EDF canonical source inventory/health accounting drift")
    if current.get("source_health_state") not in {"HEALTHY", "DEGRADED"}:
        raise SystemExit("FAIL EDF canonical source health state drift")
    if current.get("market_intelligence_only") is not True or current.get("fit_score_is_not_eligibility") is not True:
        raise SystemExit("FAIL EDF canonical intelligence boundary weakened")
    if any(current.get(flag) is not False for flag in FLAGS) or any(reconciliation.get(flag) is not False for flag in FLAGS):
        raise SystemExit("FAIL EDF canonical lane became materially authorizing")
    if current.get("publication_effect") != "NONE" or reconciliation.get("publication_effect") != "NONE":
        raise SystemExit("FAIL EDF canonical publication boundary drift")
    states = {str(row.get("observation_state") or "") for row in current.get("evidence") or []}
    if states != {"PROGRAMME_INTELLIGENCE", "PROGRAMMING_PIPELINE", "CALL_INDEX_DISCOVERY"}:
        raise SystemExit("FAIL EDF canonical observation-state drift")
    for row in current.get("evidence") or []:
        if row.get("source_health") == "HEALTHY":
            if row.get("http_status") != 200 or row.get("lkg_required") is not False:
                raise SystemExit("FAIL healthy EDF canonical receipt inconsistent")
    ...
    if reconciliation.get("material_admission_ready_for_downstream_review") is not False or reconciliation.get("lkg_reference_is_current_truth") is not False:
        raise SystemExit("FAIL EDF canonical material/LKG boundary drift")
    if current.get("source_health_state") == "DEGRADED":
        if reconciliation.get("reconciliation_state") != "CURRENT_SOURCE_HEALTH_DEGRADED_LKG_REQUIRED":
            raise SystemExit("FAIL degraded EDF current did not fail closed")
        if reconciliation.get("semantic_reconciliation_passed") is not False or reconciliation.get("semantic_change_count") != 0 or reconciliation.get("pipeline_watch_candidate") is not False or reconciliation.get("lkg_reference_required") is not True:
            raise SystemExit("FAIL degraded EDF canonical reconciliation semantics drift")
    else:
        allowed = {
            "BASELINE_CAPTURED_NON_AUTHORIZING", "NO_CHANGE",
            "EDF_PROGRAMME_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING",
        ...
        if reconciliation.get("reconciliation_state") not in allowed or reconciliation.get("semantic_reconciliation_passed") is not True:
            raise SystemExit("FAIL healthy EDF canonical reconciliation state drift")
    return {
        "programme": "EDF",
...
def stage_history(root: pathlib.Path) -> dict[str, Any]:
    current_path = root / "current" / "edf-programme-intelligence.json"
    previous_path = root / "previous" / "edf-programme-intelligence.json"
    current = load(current_path)
    history = root / "history"
    history.mkdir(parents=True, exist_ok=True)
    if current.get("source_health_state") == "HEALTHY":
        shutil.copy2(current_path, history / "edf-programme-intelligence.json")
        selected = "CURRENT_HEALTHY"
    elif previous_path.exists() and load(previous_path).get("source_health_state") == "HEALTHY":
        shutil.copy2(previous_path, history / "edf-programme-intelligence.json")
...
def main() -> int:
    ...
    root = pathlib.Path("/tmp/partener-eu-eu-direct-programme-intelligence/edf-programme")
    ...
    run_id = os.environ.get("EDF_RUN_ID") or f"{os.environ.get('GITHUB_RUN_ID','local')}-{os.environ.get('GITHUB_RUN_ATTEMPT','1')}-edf"
    current_path = root / "current" / "edf-programme-intelligence.json"
    run([
        sys.executable,
        str(ingest / "eu_direct_edf_programme_intelligence.py"),
    ...
    reconcile_cmd = [
        sys.executable,
        str(ingest / "eu_direct_edf_programme_reconcile.py"),
        str(current_path),
    ]
    previous_path = root / "previous" / "edf-programme-intelligence.json"
    if restore.get("previous_found") is True and previous_path.exists():
        reconcile_cmd += ["--previous", str(previous_path)]
    reconcile_cmd += ["--output", str(root / "current" / "edf-programme-reconciliation.json")]
    run(reconcile_cmd)
