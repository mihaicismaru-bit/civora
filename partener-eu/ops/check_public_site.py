...
uses the current Funding Concierge vNext first-time-visitor contract, and records
legacy routing separately. A deployment is current only when the public HTML and
its critical assets prove the same vNext surface that is canonical in main.
"""
...
class RedirectAudit(urllib.request.HTTPRedirectHandler):
    ...
    def redirect_request(
        ...
        self.chain.append({"http_status": code,"from_url": req.full_url,"to_url": newurl})
...
def now() -> dt.datetime: return dt.datetime.now(dt.timezone.utc)
def nowz() -> str: return now().replace(microsecond=0).isoformat().replace("+00:00", "Z")

def cache_bust(url: str, stamp: str) -> str:
    p=urllib.parse.urlsplit(url); q=urllib.parse.parse_qsl(p.query,keep_blank_values=True); q.append(("p10_probe",stamp))
    return urllib.parse.urlunsplit((p.scheme,p.netloc,p.path,urllib.parse.urlencode(q),p.fragment))


def request(url: str, limit: int = 1_000_000):
    req=urllib.request.Request(url,headers={"User-Agent":UA,"Accept":"text/html,application/javascript,text/css,*/*;q=0.8","Accept-Language":"ro-RO,ro;q=0.9,en;q=0.5","Cache-Control":"no-cache, no-store, max-age=0","Pragma":"no-cache","Connection":"close"})
    context=ssl.create_default_context(); audit=RedirectAudit(); opener=urllib.request.build_opener(urllib.request.HTTPSHandler(context=context),audit)
    with opener.open(req,timeout=30) as r:
        body=r.read(limit); return body,getattr(r,"status",200),r.geturl(),dict(r.headers.items()),audit.chain
...
def extract_stylesheet(text: str,name: str,base: str) -> str|None:
    m=re.search(rf'''(?is)<link[^>]+href=["']([^"']*{re.escape(name)}[^"']*)["']''',text)
    return urllib.parse.urljoin(base,m.group(1)) if m else None
...
def probe_asset(url: str|None, expected: tuple[str,...]) -> dict[str,Any]:
    ...
    try:
        body,code,final,_,_=request(url,600_000); text=body.decode("utf-8","ignore")
        result.update({"http_status":code,"final_url":final,"bytes":len(body),"sha256":hashlib.sha256(body).hexdigest(),"expected_marker_found":any(marker in text for marker in expected)})
...
def has_https_downgrade(endpoint: dict[str,Any]) -> bool:
    for hop in endpoint.get("redirect_chain") or []:
        if urllib.parse.urlsplit(str(hop.get("from_url") or "")).scheme.lower()=="https" and urllib.parse.urlsplit(str(hop.get("to_url") or "")).scheme.lower()=="http": return True
    requested=urllib.parse.urlsplit(str(endpoint.get("requested_url") or endpoint.get("url") or "")).scheme.lower()
    return requested=="https" and endpoint.get("final_scheme")=="http"


def assess_transport(by_id: dict[str,dict[str,Any]]) -> dict[str,bool]:
    https=by_id["custom_https"]; http=by_id["custom_http"]; pages=by_id["pages_origin"]
    custom_https_verified=bool(https.get("content_verified") and https.get("final_scheme")=="https" and not https.get("error") and not has_https_downgrade(https))
    http_redirects_to_https=bool(http.get("redirect_chain") and http.get("content_verified") and http.get("final_scheme")=="https" and not has_https_downgrade(http))
    pages_https_preserved=bool(pages.get("content_verified") and pages.get("final_scheme")=="https" and not has_https_downgrade(pages))
    return {"custom_https_verified":custom_https_verified,"http_redirects_to_https":http_redirects_to_https,"pages_https_preserved":pages_https_preserved,"secure_transport_verified":bool(custom_https_verified and http_redirects_to_https and pages_https_preserved)}
