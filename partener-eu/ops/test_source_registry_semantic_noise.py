...
def main() -> int:
    ...
    <html><body>
      <article>
        ...
        <div class="views">92065 de afişări</div>
      </article>
    </body></html>
    """
    counter_only = base.replace("92065 de afişări", "92174 de afișări")
    ...
    assert digest(base) == digest(counter_only), "view-count telemetry must not change semantic hash"
    ...
    binary = b"92065 de afisari\x00material"
    assert semantic_bytes(binary, "application/pdf") == binary, "non-HTML sources must remain byte-exact"

    print("PASS source-registry semantic noise regression: counters ignored; substantive facts preserved")
    return 0
