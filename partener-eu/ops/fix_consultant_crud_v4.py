...
# Safe IDs on every browser / transport context.
text2 = text.replace(
...
# State tombstones are required so deleted seed/example clients cannot resurrect.
text2 = re.sub(
...
# Merge seeded clients only when not explicitly deleted.
...
# Clear, always-visible portfolio action.
...
# New-client transition: persist UI state, clear call context, then focus name.
...
text2 = re.sub(r"root\.querySelectorAll\('\[data-cw3-new-client\]'\)\.forEach\(b=>b\.onclick=.*?\);", new_handler, text, count=1)
if text2 != text: text, changed = text2, True

# Atomic save with duplicate-CUI warning and tombstone removal.
...
text2, n = re.subn(r"const form=document\.getElementById\('cw3ClientForm'\);if\(form\)form\.onsubmit=.*?;\n root\.querySelectorAll\('\[data-cw3-cancel-new\]'\)", save_handler+"\n root.querySelectorAll('[data-cw3-cancel-new]')", text, count=1, flags=re.S)
if n: text, changed = text2, True

# Delete profile data first; attachment cleanup is best-effort and can never abort deletion.
...
text2 = re.sub(r"root\.querySelectorAll\('\[data-cw3-delete-client\]'\)\.forEach\(b=>b\.onclick=async\(\)=>\{.*?\}\);", delete_handler, text, count=1, flags=re.S)
if text2 != text: text, changed = text2, True

# Removing all examples also persists tombstones immediately.
...
text2 = re.sub(r"root\.querySelectorAll\('\[data-cw3-remove-demo\]'\)\.forEach\(b=>b\.onclick=.*?\);", demo_handler, text, count=1, flags=re.S)
if text2 != text: text, changed = text2, True
...
if changed:
    ...
    if INDEX.exists():
        ...
        index = re.sub(r'consultant-workspace-v3\.js\?v=[^"\']+', 'consultant-workspace-v3.js?v=20260815-2030', index)
        index = re.sub(r'consultant-workspace-v3\.css\?v=[^"\']+', 'consultant-workspace-v3.css?v=20260815-2030', index)
        index = re.sub(r'consultant-onboarding-v3\.js\?v=[^"\']+', 'consultant-onboarding-v3.js?v=20260815-2030', index)
