...
import eu_direct_edf_programme_intelligence as source_mod
import eu_direct_edf_programme_reconcile as rec_mod

REGISTRY_PATH = ROOT / "ingest" / "eu_direct_edf_programme_intelligence_registry.json"
...
class EDFReconcileTests(unittest.TestCase):
    def test_baseline_non_authorizing(self):
        cur = snapshot("cur", "2026-09-03T12:00:00+00:00")
    ...
    def test_semantic_change_becomes_pipeline_watch_only(self):
        prev = snapshot("prev", "2026-09-03T11:00:00+00:00")
        cur = snapshot("cur", "2026-09-03T12:00:00+00:00", " amended official programme wording")
        rec = rec_mod.reconcile(cur, prev)
        self.assertEqual(rec["reconciliation_state"], "EDF_PROGRAMME_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING")
    ...
    def test_degraded_current_requires_lkg(self):
        prev = snapshot("prev", "2026-09-03T11:00:00+00:00")
        calls = {"n": 0}
        healthy = fetch_for()
        ...
        cur["fetched_at"] = "2026-09-03T12:00:00+00:00"
        rec = rec_mod.reconcile(cur, prev)
    ...
    def test_recovery_from_degraded_previous_refreshes_baseline(self):
        ...
        prev["fetched_at"] = "2026-09-03T11:00:00+00:00"
        cur = snapshot("cur", "2026-09-03T12:00:00+00:00")
        rec = rec_mod.reconcile(cur, prev)
    ...
    def test_identity_drift_rejected(self):
        prev = snapshot("prev", "2026-09-03T11:00:00+00:00")
        cur = snapshot("cur", "2026-09-03T12:00:00+00:00")
        prev["evidence"][0]["authority_url"] = "https://defence-industry-space.ec.europa.eu/drift"
        with self.assertRaises(ValueError):
            rec_mod.reconcile(cur, prev)

    def test_future_previous_rejected(self):
        prev = snapshot("prev", "2026-09-03T13:00:00+00:00")
        cur = snapshot("cur", "2026-09-03T12:00:00+00:00")
        with self.assertRaises(ValueError):
            rec_mod.reconcile(cur, prev)
