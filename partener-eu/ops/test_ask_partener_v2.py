...
assert 'function beneficiaryCompatibility(' in js
assert 'const actual=detectedKeys(p.audience,BENEFICIARY_KEYS)' in js
assert "interpretation.beneficiary.some(key=>actual.includes(key))?'MATCH':'MISMATCH'" in js
assert 'beneficiaryConflict:true' in js
assert '!x.beneficiaryConflict' in js
assert 'Tipul de beneficiar cerut a fost înțeles, dar eligibilitatea acestui dosar nu este încă suficient de structurată pentru confirmarea potrivirii.' in js
assert 'beneficiar ori zonă structurată incompatibilă' in js
assert 'beneficiar sau zonă structurată incompatibilă' in js
