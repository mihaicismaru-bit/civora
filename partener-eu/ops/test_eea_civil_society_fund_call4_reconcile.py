...
from eea_civil_society_fund_call4_exact import EXACT_URL, INDEX_URL, collect_exact, sha256_json
from eea_civil_society_fund_call4_reconcile import reconcile
...
def degraded_fetch(url: str, *, timeout: float):
    if url == EXACT_URL:
        raise URLError("synthetic Call 4 authority outage")
...
def main() -> int:
    ...
    changed["exact_semantics"]["budget_candidate"] = "EUR 4,478,019"
    changed["budget_candidate"] = "EUR 4,478,019"
    changed["exact_semantic_fingerprint"] = sha256_json(changed["exact_semantics"])
    changed_rec = reconcile(changed, previous)
    assert changed_rec["reconciliation_state"] == "EEA_CSF_CALL4_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
    ...
    try:
        ...
        raise AssertionError("Call 4 accepted equal-time previous evidence")
    ...
    try:
        ...
        raise AssertionError("Call 4 reconciliation accepted identity drift")
    ...
    try:
        ...
        raise AssertionError("Call 4 reconciliation accepted authorization widening")

    print({
        "status": "PASS",
        "reconciler": "EEA_CSF_RO_CALL4_RECONCILIATION",
