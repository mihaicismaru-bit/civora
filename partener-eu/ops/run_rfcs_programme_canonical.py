...
SNAPSHOT_SCHEMA = "PARTENER_EU_RFCS_PROGRAMME_INTELLIGENCE_V1"
RECONCILIATION_SCHEMA = "PARTENER_EU_RFCS_PROGRAMME_RECONCILIATION_V1"
PROGRAMME_ID = "RFCS"
ARTIFACT_PREFIXES = (
    "partener-eu-eu-direct-programme-intelligence-",
    "partener-eu-rfcs-programme-history-",
    "partener-eu-rfcs-programme-proof-",
)
...
def candidate_priority(path: pathlib.Path) -> tuple[int, str]:
    ...
    if "/history/" in text or text.endswith("/history/rfcs-programme-intelligence.json"):
        return (0, text)
    if "/current/" in text:
        return (1, text)
    return (2, text)
...
def restore_previous(root: pathlib.Path, registry: dict[str, Any]) -> dict[str, Any]:
    ...
    scratch = pathlib.Path("/tmp/partener-eu-rfcs-canonical-history-scan")
    ...
    for _, artifact_id, artifact_name in rows:
        ...
        candidates = sorted(unpack.rglob("rfcs-programme-intelligence.json"), key=candidate_priority)
        for candidate_path in candidates:
            ...
            if inventory != expected:
                continue
            dump(previous_dir / "rfcs-programme-intelligence.json", candidate)
            metadata.update({
                "previous_found": True,
...
def enforce_boundary(root: pathlib.Path) -> dict[str, Any]:
    current = load(root / "current" / "rfcs-programme-intelligence.json")
    reconciliation = load(root / "current" / "rfcs-programme-reconciliation.json")
    if current.get("schema") != SNAPSHOT_SCHEMA or current.get("source_family") != "EU_DIRECT" or current.get("programme_id") != PROGRAMME_ID:
        raise SystemExit("FAIL RFCS canonical snapshot identity drift")
    if current.get("source_count") != 4 or int(current.get("healthy_source_count") or 0) + int(current.get("degraded_source_count") or 0) != 4:
        raise SystemExit("FAIL RFCS canonical source inventory/health accounting drift")
    if current.get("source_health_state") not in {"HEALTHY", "DEGRADED"}:
        raise SystemExit("FAIL RFCS canonical source health state drift")
    if current.get("market_intelligence_only") is not True or current.get("fit_score_is_not_eligibility") is not True or current.get("partner_intelligence_is_not_call_eligibility") is not True:
        raise SystemExit("FAIL RFCS canonical intelligence boundary weakened")
    if current.get("romania_programme_fit") != "EU_MEMBER_STATE_APPLICANT_POOL_NON_AUTHORIZING":
        raise SystemExit("FAIL RFCS canonical Romania fit boundary drift")
    if any(current.get(flag) is not False for flag in FLAGS) or any(reconciliation.get(flag) is not False for flag in FLAGS):
        raise SystemExit("FAIL RFCS canonical lane became materially authorizing")
    if current.get("publication_effect") != "NONE" or reconciliation.get("publication_effect") != "NONE":
        raise SystemExit("FAIL RFCS canonical publication boundary drift")
    states = {str(row.get("observation_state") or "") for row in current.get("evidence") or []}
    if states != {"PROGRAMME_INTELLIGENCE", "PROGRAMMING_PIPELINE", "CALL_INDEX_DISCOVERY", "APPLICANT_FIT_INTELLIGENCE"}:
        raise SystemExit("FAIL RFCS canonical observation-state drift")
    for row in current.get("evidence") or []:
        if row.get("source_health") == "HEALTHY":
            if row.get("http_status") != 200 or row.get("lkg_required") is not False:
                raise SystemExit("FAIL healthy RFCS canonical receipt inconsistent")
    ...
    if reconciliation.get("material_admission_ready_for_downstream_review") is not False or reconciliation.get("lkg_reference_is_current_truth") is not False:
        raise SystemExit("FAIL RFCS canonical material/LKG boundary drift")
    if current.get("source_health_state") == "DEGRADED":
        if reconciliation.get("reconciliation_state") != "CURRENT_SOURCE_HEALTH_DEGRADED_LKG_REQUIRED":
            raise SystemExit("FAIL degraded RFCS current did not fail closed")
        if reconciliation.get("semantic_reconciliation_passed") is not False or reconciliation.get("semantic_change_count") != 0 or reconciliation.get("pipeline_watch_candidate") is not False or reconciliation.get("lkg_reference_required") is not True:
            raise SystemExit("FAIL degraded RFCS canonical reconciliation semantics drift")
    else:
        allowed = {
            "BASELINE_CAPTURED_NON_AUTHORIZING", "NO_CHANGE",
            "RFCS_PROGRAMME_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING",
        ...
        if reconciliation.get("reconciliation_state") not in allowed or reconciliation.get("semantic_reconciliation_passed") is not True:
            raise SystemExit("FAIL healthy RFCS canonical reconciliation state drift")
    return {
        "programme": "RFCS",
...
def stage_history(root: pathlib.Path) -> dict[str, Any]:
    current_path = root / "current" / "rfcs-programme-intelligence.json"
    previous_path = root / "previous" / "rfcs-programme-intelligence.json"
    current = load(current_path)
    history = root / "history"
    history.mkdir(parents=True, exist_ok=True)
    if current.get("source_health_state") == "HEALTHY":
        shutil.copy2(current_path, history / "rfcs-programme-intelligence.json")
        selected = "CURRENT_HEALTHY"
    elif previous_path.exists() and load(previous_path).get("source_health_state") == "HEALTHY":
        shutil.copy2(previous_path, history / "rfcs-programme-intelligence.json")
...
def main() -> int:
    ...
    registry_path = ingest / "eu_direct_rfcs_programme_intelligence_registry.json"
    registry = load(registry_path)
    root = pathlib.Path("/tmp/partener-eu-eu-direct-programme-intelligence/rfcs-programme")
    ...
    run_id = os.environ.get("RFCS_RUN_ID") or f"{os.environ.get('GITHUB_RUN_ID','local')}-{os.environ.get('GITHUB_RUN_ATTEMPT','1')}-rfcs"
    current_path = root / "current" / "rfcs-programme-intelligence.json"
    run([
        sys.executable,
        str(ingest / "eu_direct_rfcs_programme_intelligence.py"),
    ...
    reconcile_cmd = [
        sys.executable,
        str(ingest / "eu_direct_rfcs_programme_reconcile.py"),
        str(current_path),
    ]
    previous_path = root / "previous" / "rfcs-programme-intelligence.json"
    if restore.get("previous_found") is True and previous_path.exists():
        reconcile_cmd += ["--previous", str(previous_path)]
    reconcile_cmd += ["--output", str(root / "current" / "rfcs-programme-reconciliation.json")]
    run(reconcile_cmd)
