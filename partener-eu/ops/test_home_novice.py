...
goto=(WEB/'home-go-to-v2.js').read_text(encoding='utf-8')
css=(WEB/'home-novice-v1.css').read_text(encoding='utf-8')
decision=(WEB/'decision-intelligence-v2.js').read_text(encoding='utf-8')

for token in ('home-novice-v1.css','home-novice-v1.js','home-go-to-v2.js','decision-intelligence-v2.js'):
...
for phrase in ('Vezi apelurile deschise','Găsește finanțări pentru mine','Nu trebuie să știi numele programului','cine poate aplica','câți bani','ce trebuie făcut mai departe'):
...
for phrase in ('Ai o investiție în minte?','Sau caută direct după ideea ta','Vezi ce s-a schimbat de la ultima verificare','Prima dată aici?'):
    assert phrase in goto, f'missing go-to homepage behavior: {phrase}'
