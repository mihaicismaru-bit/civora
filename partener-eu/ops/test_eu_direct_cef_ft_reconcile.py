...
from eu_direct_cef_ft_exact import sha256_json
from eu_direct_cef_ft_reconcile import reconcile, validate_receipt

REF = "CEF-T-2026-AFIFGEN-COSTS"


def evidence(ts="2026-09-01T18:00:00+00:00", state="OPEN_CALL", status="Open", deadline="2027-01-15T17:00:00Z"):
    authority_url = "https://ec.europa.eu/info/funding-tenders/opportunities/portal/screen/opportunities/topic-details/" + REF
    semantics = {
        "identifier": REF,
        "call_identifier": "CEF-T-2026-AFIFGEN",
        "title": "Synthetic CEF transport topic",
        "programme_reference": "43251567",
...
def main():
    ...
    same = evidence("2026-09-01T19:00:00+00:00")
    no_change = reconcile(same, current)
    assert no_change["reconciliation_state"] == "NO_CHANGE"
    assert no_change["semantic_change_count"] == 0

    closed = evidence("2026-09-01T20:00:00+00:00", state="CLOSED_CALL", status="Closed")
    changed = reconcile(closed, same)
    assert changed["reconciliation_state"] == "CEF_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
    ...
    print("eu_direct_cef_ft_reconcile regression: PASS")
