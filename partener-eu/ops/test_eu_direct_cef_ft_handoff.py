...
from eu_direct_cef_ft_handoff import (  # noqa: E402
    CURRENT_MODE,
...
from eu_direct_cef_ft_exact import SCHEMA as CEF_SCHEMA, PARSER_VERSION as CEF_PARSER  # noqa: E402
...
def cef_row(reference="CEF-T-2026-TEST-WORKS"):
    return {
        "programme_family_normalized": "CEF",
...
def previous(reference="CEF-T-2026-TEST-WORKS"):
    # Minimal object accepted by the production validator. The exact-semantic
    # fingerprint is bound deterministically below.
    from eu_direct_cef_ft_exact import MATERIAL_FLAGS, sha256_json

    semantics = {
        "identifier": reference,
        "call_identifier": "CEF-T-2026-TEST",
        "title": "Synthetic CEF test",
    ...
    obj = {
        ...
        "programme_label_official": "Connecting Europe Facility (CEF)",
        "deadline_candidate": None,
        "budget_candidate": None,
...
def main() -> int:
    current = resolve_handoff(taxonomy([cef_row()]), run_id="current")
    assert current["observation_state"] == CURRENT_MODE
    assert current["target_reference"] == "CEF-T-2026-TEST-WORKS"
    ...
    different_prev = previous("CEF-T-2025-OTHER-WORKS")
    current_with_other_history = resolve_handoff(taxonomy([cef_row()]), previous=different_prev, run_id="current-other-history")
