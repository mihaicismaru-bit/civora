...
def degraded_hosted_state() -> dict:
    return {
        ...
        "lastRun": {
            ...
            "roots": [
                {
                    "target": module.PDDS_PRIORITY_SEED,
                    "ok": False,
        ...
        "items": [
            {
                "id": "mysmis-1",
                "url": "https://reporting.mysmis2021.gov.ro/example",
                "title": "MySMIS direct evidence",
                "verification": "CANONICAL_OFFICIAL_FETCH",
                "retrievalTransport": "direct-canonical",
            }
        ],
        "runs": [],
    }
...
def fresh_ro_corpus() -> dict:
    return {
        ...
        "lastRun": {
            ...
            "roots": [
                {
                    "root": module.PDDS_PRIORITY_SEED,
                    "ok": True,
                    "status": 200,
                    "finalUrl": module.PDDS_PRIORITY_SEED,
                }
            ],
        },
        "pages": [
...
def test_fresh_ro_direct_route_wins_over_hosted_egress_failure() -> None:
    state, changed = module.normalize_state(
        degraded_hosted_state(),
        fresh_ro_corpus(),
        reference_time=NOW,
    )
...
def test_stale_ro_corpus_does_not_mask_hosted_failure() -> None:
    ...
    state, _ = module.normalize_state(
        degraded_hosted_state(),
        corpus,
        reference_time=NOW,
    )
