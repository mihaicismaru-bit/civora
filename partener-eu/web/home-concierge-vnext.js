...
function dateText(v){
 const d=parseDate(v);if(!d)return String(v||'');
...
function isUpcoming(d){
 const s=currentStatus(d);return ['EXPECTED','ANNOUNCED','UPCOMING','PREPARE_NOW'].includes(s)&&String(d.publicationState||'PUBLISHABLE').toUpperCase()==='PUBLISHABLE';
}
function isConsultation(d){return currentStatus(d)==='PUBLIC_CONSULTATION'&&String(d.publicationState||'PUBLISHABLE').toUpperCase()==='PUBLISHABLE'};
function score(d){
...
function card(d,mode='open'){
 ...
 const meta=[];
 if(funding?.value)meta.push(String(funding.value));
 if(deadline?.value)meta.push(`până la ${dateText(deadline.value)}`);
...
function newsRow(n){
 const label=String(n.kind||'ACTUALIZARE').replaceAll('_',' ');
...
function render(){
 ...
 const home=!!(main&&hero&&document.querySelector('.diHome'));
...
window.addEventListener('load',()=>setTimeout(render,520),{once:true});
