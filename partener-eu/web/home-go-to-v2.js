...
function openHub(query='',tab='dossiers'){
  const nav=qs('[data-decisionnav]');
  if(nav)nav.click();
  setTimeout(()=>{
    const tabBtn=qs(`[data-di-tab="${tab}"]`);
...
function localGeneratedAt(){
  if(!P.generatedAt)return 'actualizare automată';
  try{
    return new Intl.DateTimeFormat('ro-RO',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}).format(new Date(P.generatedAt));
  }catch{return 'actualizare recentă'}
}
...
function enhanceHero(){
  ...
  if(actions&&!qs('[data-goto-search]',hero)){
    ...
    search.className='gotoSearch'; search.dataset.gotoSearch='1';
    ...
    search.addEventListener('submit',e=>{e.preventDefault();const q=qs('#gotoQ',search)?.value.trim();if(q)openHub(q,'dossiers')});
  }
  ...
  if(proof&&!qs('[data-goto-pulse]',hero)){
    const pulse=document.createElement('div'); pulse.className='gotoPulse'; pulse.dataset.gotoPulse='1';
    const summary=P.summary||{};
    pulse.innerHTML=`<span><b>${summary.openCount??'—'}</b> apeluri confirmate deschise</span><span><b>${summary.prepareCount??'—'}</b> oportunități de pregătit</span><span><b>${summary.dossierCount??'—'}</b> dosare urmărite</span><small>Actualizat ${localGeneratedAt()}</small>`;
    proof.insertAdjacentElement('afterend',pulse);
  }
}
...
function addReturnPath(){
  const home=qs('.diHome'); if(!home||qs('[data-goto-return]'))return;
  const summary=qs('.diHomeSummary',home); if(!summary)return;
  const strip=document.createElement('section'); strip.className='gotoReturn'; strip.dataset.gotoReturn='1';
  const count=P.summary?.newsCount??0;
  strip.innerHTML=`<div><span class="gotoKicker">Revii pe PARTENER.EU?</span><b>Vezi ce s-a schimbat de la ultima verificare.</b><small>${count?`${count} schimbări cu utilitate sunt în fluxul curent.`:'Dacă nu există schimbări materiale, nu umplem pagina cu zgomot.'}</small></div><button data-goto-news>Vezi schimbările importante →</button>`;
  summary.insertAdjacentElement('beforebegin',strip);
  qs('[data-goto-news]',strip)?.addEventListener('click',()=>openHub('','news'));
}
...
function addPromise(){
  ...
  section.innerHTML='<div><span class="gotoKicker">De ce PARTENER.EU</span><h2>De la „am auzit că sunt niște fonduri” la o decizie clară.</h2></div><div class="gotoPromiseGrid"><article><b>Găsești</b><p>oportunități după profilul tău, nu după jargonul instituției.</p></article><article><b>Înțelegi</b><p>cine poate aplica, pentru ce, cu ce buget și până când.</p></article><article><b>Verifici</b><p>sursa oficială și vezi separat informația încă neconfirmată.</p></article><article><b>Acționezi</b><p>cu documente, pași următori și schimbări importante într-un singur loc.</p></article></div>';
  summary.insertAdjacentElement('afterend',section);
}

function addPopularSearches(){
  const entry=qs('.noviceEntry'); if(!entry||qs('[data-goto-popular]',entry))return;
  const block=document.createElement('div');block.className='gotoPopular';block.dataset.gotoPopular='1';
  const items=['panouri fotovoltaice','digitalizare IMM','utilaje și producție','turism','agricultură','școală și educație'];
  block.innerHTML='<span>Căutări utile:</span>'+items.map(x=>`<button data-goto-query="${x}">${x}</button>`).join('');
  entry.appendChild(block);
  block.querySelectorAll('[data-goto-query]').forEach(btn=>btn.addEventListener('click',()=>openHub(btn.dataset.gotoQuery,'dossiers')));
}
