...
SCHEMA = "PARTENER_EU_RFCS_PROGRAMME_INTELLIGENCE_V1"
REGISTRY_SCHEMA = "PARTENER_EU_RFCS_PROGRAMME_INTELLIGENCE_REGISTRY_V1"
PARSER_VERSION = "EU_DIRECT_RFCS_PROGRAMME_INTELLIGENCE_V1"
ALLOWED_HOSTS = {"research-and-innovation.ec.europa.eu", "rea.ec.europa.eu"}
MATERIAL_FLAGS = (
...
def validate_registry(registry: Mapping[str, Any]) -> None:
    if registry.get("schema") != REGISTRY_SCHEMA:
        raise ValueError("RFCS registry schema drift")
    if registry.get("source_family") != "EU_DIRECT" or registry.get("programme_id") != "RFCS":
        raise ValueError("RFCS programme identity drift")
    policy = registry.get("policy") or {}
    for flag in MATERIAL_FLAGS:
        if policy.get(flag) is not False:
            raise ValueError(f"RFCS registry became authorizing: {flag}")
    for key in (
        ...
        if policy.get(key) is not True:
            raise ValueError(f"RFCS policy weakened: {key}")
    sources = registry.get("sources") or []
    if len(sources) != 4:
        raise ValueError("RFCS source inventory drift")
    ids = set()
    for row in sources:
        source_id = str(row.get("source_id") or "")
        if not source_id or source_id in ids:
            raise ValueError("RFCS source identity duplicated or missing")
        ids.add(source_id)
        ...
        if parsed.scheme != "https" or (parsed.hostname or "").casefold() not in ALLOWED_HOSTS:
            raise ValueError("RFCS authority URL drift")
        if row.get("observation_state") not in ALLOWED_STATES:
            raise ValueError("RFCS observation state became call state")
        if not row.get("required_markers"):
            raise ValueError("RFCS source markers missing")
    fit = registry.get("applicant_fit") or {}
    if fit.get("fit_is_not_eligibility") is not True:
        raise ValueError("RFCS fit became eligibility")
    if fit.get("romania_programme_fit") != "EU_MEMBER_STATE_APPLICANT_POOL_NON_AUTHORIZING":
        raise ValueError("RFCS Romania fit boundary drift")
    partner = registry.get("partner_intelligence") or {}
    if partner.get("partner_intelligence_is_not_call_eligibility") is not True:
        raise ValueError("RFCS partner intelligence became eligibility")
...
def fetch(url: str, timeout: float = 30.0) -> tuple[bytes, dict[str, Any]]:
    ...
    with urllib.request.urlopen(req, timeout=timeout) as response:
        ...
        if len(raw) > 3_000_000:
            raise ValueError("RFCS source exceeds 3 MB")
        return raw, {
            "requested_url": url,
...
def collect(registry: Mapping[str, Any], run_id: str, fetcher=fetch) -> dict[str, Any]:
    ...
    snapshot = {
        ...
        "romania_programme_fit": (registry.get("applicant_fit") or {}).get("romania_programme_fit"),
    ...
    snapshot["semantic_fingerprint"] = sha256_json({
        ...
        "romania_programme_fit": snapshot["romania_programme_fit"],
    })
    return snapshot
