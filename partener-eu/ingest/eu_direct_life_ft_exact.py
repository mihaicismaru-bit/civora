#!/usr/bin/env python3
"""Exact current Funding & Tenders evidence for one LIFE topic.

Consumes the programme-wide taxonomy only as a non-authorizing handoff pointer,
then re-queries the official EC Search/Facet endpoints for one exact LIFE-* topic
and verifies the exact official topic URL. No material field is authorized here.
"""
...
class ExactLifeConflict(ValueError):
    pass
...
def validate_reference(reference: str) -> str:
    ...
    if not REF_RE.fullmatch(value):
        raise ValueError(f"not an explicit LIFE topic reference: {reference!r}")
    return value
...
def _is_life_label(label: str) -> bool:
    ...
    return "environment and climate action" in token and "life" in token
...
def select_life_candidate(taxonomy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in taxonomy.get("records") or []:
        if row.get("programme_family_normalized") != "LIFE":
            continue
    ...
    if not candidates:
        raise ValueError("programme taxonomy contains no LIFE candidate")
...
def collect_exact(
    ...
    *,
    ...
    for row in primary:
        ...
        if not programme_label or not _is_life_label(programme_label):
            raise ValueError(f"exact record is not proven to belong to LIFE: {programme_ref!r} {programme_label!r}")
    ...
    if len(signatures) != 1:
        raise ExactLifeConflict(f"conflicting exact LIFE records for {reference}: {len(signatures)} material variants")
    ...
    if readback.get("verified") is not True:
        raise ValueError(f"exact LIFE topic authority readback failed for {reference}: {readback.get('error')}")
    ...
    batch = normalize_payload(
        ...
        run_id=run_id,
        verified_authority_urls=[authority_url],
    )
    records = [row for row in batch.get("records") or [] if str(row.get("identifier") or "").upper() == reference]
    if len(records) != 1:
        raise ValueError(f"exact LIFE normalizer returned {len(records)} records for {reference}")
    normalized = records[0]
    if normalized.get("authority_url_verified") is not True:
        raise ValueError("exact LIFE authority verification was lost during normalization")
    ...
    if output_dir:
        ...
        (output_dir / "ft-life-search-response.json").write_bytes(search_raw)
        (output_dir / "ft-life-facet-response.json").write_bytes(facet_raw)
        (output_dir / "ft-life-exact-evidence.json").write_text(
...
def validate_evidence(evidence: Mapping[str, Any]) -> None:
    if evidence.get("schema") != SCHEMA or evidence.get("parser_version") != PARSER_VERSION:
        raise ValueError("LIFE exact evidence schema/parser drift")
    ...
    if evidence.get("source_family") != SOURCE_FAMILY or evidence.get("programme_family") != PROGRAMME_FAMILY:
        raise ValueError("LIFE exact evidence family drift")
    if evidence.get("authority_class") != AUTHORITY_CLASS:
        raise ValueError("LIFE exact evidence authority class drift")
    if evidence.get("authority_url") != ft.topic_url(reference) or evidence.get("authority_url_verified") is not True:
        raise ValueError("LIFE exact evidence lacks verified exact topic authority")
    ...
    if readback.get("verified") is not True or readback.get("url") != evidence.get("authority_url"):
        raise ValueError("LIFE exact readback binding invalid")
    label = str(evidence.get("programme_label_official") or "")
    if not _is_life_label(label):
        raise ValueError("LIFE exact evidence lost official programme proof")
    if evidence.get("candidate_state") not in {"OPEN_CALL", "FORTHCOMING_CALL", "CLOSED_CALL", "UNKNOWN"}:
        raise ValueError("LIFE exact candidate state unsupported")
    if not evidence.get("status_label"):
        raise ValueError("LIFE exact evidence lacks resolved official status label")
    ...
    if not isinstance(semantics, dict) or sha256_json(semantics) != evidence.get("exact_semantic_fingerprint"):
        raise ValueError("LIFE exact semantic fingerprint mismatch")
    for key in MATERIAL_FLAGS:
        if evidence.get(key) is not False:
            raise ValueError(f"LIFE exact evidence attempted authorization: {key}")
    if evidence.get("publication_effect") != "NONE" or evidence.get("canonical_corpus_mutation") is not False:
        raise ValueError("LIFE exact evidence crossed publication boundary")
    if evidence.get("semantic_reconciliation_required") is not True or evidence.get("field_scoped_material_admission_required") is not True:
        raise ValueError("LIFE exact evidence skipped downstream gates")
    for receipt_key in ("search_receipt", "facet_receipt"):
        ...
        if not re.fullmatch(r"[0-9a-f]{64}", str(receipt.get("sha256") or "")):
            raise ValueError(f"LIFE exact evidence missing immutable {receipt_key}")
...
def main() -> int:
    ...
    parser.add_argument("--run-id", default="life-ft-exact-live")
