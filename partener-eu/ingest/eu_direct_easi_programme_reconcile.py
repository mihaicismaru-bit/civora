...
SCHEMA = "PARTENER_EU_EASI_PROGRAMME_RECONCILIATION_V1"
CURRENT_SCHEMA = "PARTENER_EU_EASI_PROGRAMME_INTELLIGENCE_V1"
RECONCILER_VERSION = "EU_DIRECT_EASI_PROGRAMME_RECONCILE_V1"
MATERIAL_FLAGS = (
...
def validate_snapshot(snapshot: Mapping[str, Any], label: str) -> None:
    if snapshot.get("schema") != CURRENT_SCHEMA:
        raise ValueError(f"{label} EaSI snapshot schema drift")
    if snapshot.get("source_family") != "EU_DIRECT" or snapshot.get("programme_id") != "ESF_PLUS_EASI":
        raise ValueError(f"{label} EaSI identity drift")
    if snapshot.get("source_count") != 4 or len(snapshot.get("evidence") or []) != 4:
        raise ValueError(f"{label} EaSI source inventory drift")
    if snapshot.get("healthy_source_count", 0) + snapshot.get("degraded_source_count", 0) != 4:
        raise ValueError(f"{label} EaSI health accounting drift")
    if snapshot.get("source_health_state") not in {"HEALTHY", "DEGRADED"}:
        raise ValueError(f"{label} EaSI health state invalid")
    if snapshot.get("market_intelligence_only") is not True:
        raise ValueError(f"{label} EaSI market-intelligence boundary weakened")
    if snapshot.get("fit_score_is_not_eligibility") is not True:
        raise ValueError(f"{label} EaSI fit boundary weakened")
    if snapshot.get("partner_intelligence_is_not_call_eligibility") is not True:
        raise ValueError(f"{label} EaSI partner boundary weakened")
    for flag in MATERIAL_FLAGS:
        if snapshot.get(flag) is not False:
            raise ValueError(f"{label} EaSI snapshot became authorizing: {flag}")
    if snapshot.get("publication_effect") != "NONE":
        raise ValueError(f"{label} EaSI publication boundary drift")
    if len(str(snapshot.get("semantic_fingerprint") or "")) != 64:
        raise ValueError(f"{label} EaSI semantic fingerprint missing")
    if not snapshot.get("fetched_at"):
        raise ValueError(f"{label} EaSI fetched_at missing")
...
def semantic_changes(previous: Mapping[str, Any], current: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for key in ("applicant_fit_tags", "partner_intelligence_tags"):
...
def reconcile(current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if previous is not None:
        ...
        if not previous_identity_match:
            raise ValueError("EaSI previous source/programme identity mismatch")
        if parse_time(str(previous["fetched_at"])) >= parse_time(str(current["fetched_at"])):
            previous_newer_rejected = True
            raise ValueError("EaSI previous snapshot is not older than current")
        previous_valid = True
    ...
    if not current_healthy:
        ...
        if changes:
            state = "EASI_PROGRAMME_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
    ...
    result = {
        ...
        "previous_snapshot_newer_rejected": previous_newer_rejected,
