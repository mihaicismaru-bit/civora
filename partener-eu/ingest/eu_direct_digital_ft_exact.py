#!/usr/bin/env python3
"""Exact current Funding & Tenders evidence for one Digital Europe topic.
...
REF_RE = re.compile(r"^DIGITAL-[A-Z0-9]+(?:-[A-Z0-9]+)+$", re.IGNORECASE)
DIRECT_TYPES = {"1", "2"}
MATERIAL_FLAGS = (
    "material_fact_use",
...
class ExactDigitalConflict(ValueError):
    pass
...
def validate_reference(reference: str) -> str:
    ...
    if not REF_RE.fullmatch(value):
        raise ValueError(f"not an explicit Digital Europe topic reference: {reference!r}")
    return value
...
def _is_digital_label(label: str) -> bool:
    token = re.sub(r"[^a-z0-9]+", " ", label.casefold()).strip()
    return "digital europe programme" in token
...
def select_digital_candidate(taxonomy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not candidates:
        raise ValueError("programme taxonomy contains no Digital Europe candidate")
...
def collect_exact(
    ...
    *,
    ...
    for row in primary:
        ...
        if not programme_label or not _is_digital_label(programme_label):
            raise ValueError(
                f"exact record is not proven to belong to Digital Europe: {programme_ref!r} {programme_label!r}"
            )
    ...
    if len(signatures) != 1:
        raise ExactDigitalConflict(
            f"conflicting exact Digital Europe records for {reference}: {len(signatures)} material variants"
        )
    ...
    if output_dir:
        ...
        (output_dir / "ft-digital-search-response.json").write_bytes(search_raw)
        (output_dir / "ft-digital-facet-response.json").write_bytes(facet_raw)
        (output_dir / "ft-digital-exact-evidence.json").write_text(
...
def validate_evidence(evidence: Mapping[str, Any]) -> None:
    ...
    for key in MATERIAL_FLAGS:
        if evidence.get(key) is not False:
            raise ValueError(f"Digital Europe exact evidence attempted authorization: {key}")
    if evidence.get("publication_effect") != "NONE" or evidence.get("canonical_corpus_mutation") is not False:
        raise ValueError("Digital Europe exact evidence crossed publication boundary")
    if evidence.get("semantic_reconciliation_required") is not True or evidence.get("field_scoped_material_admission_required") is not True:
        raise ValueError("Digital Europe exact evidence skipped downstream gates")
