#!/usr/bin/env python3
"""Exact current Funding & Tenders evidence for one CEF topic.

Consumes programme-wide taxonomy only as a non-authorizing handoff pointer,
then re-queries official EC Search/Facet endpoints for one exact CEF-* topic and
verifies the exact official topic URL. No material field is authorized here.
"""
...
SCHEMA = "PARTENER_EU_CEF_FT_EXACT_EVIDENCE_V1"
PARSER_VERSION = "EU_DIRECT_CEF_FT_EXACT_V1"
...
REF_RE = re.compile(r"^CEF-[A-Z0-9]+(?:-[A-Z0-9]+)+$", re.IGNORECASE)
DIRECT_TYPES = {"1", "2"}
MATERIAL_FLAGS = (
    "material_fact_use",
...
class ExactCefConflict(ValueError):
    pass
...
def validate_reference(reference: str) -> str:
    ...
    if not REF_RE.fullmatch(value):
        raise ValueError(f"not an explicit CEF topic reference: {reference!r}")
    return value
...
def _is_cef_label(label: str) -> bool:
    token = re.sub(r"[^a-z0-9]+", " ", label.casefold()).strip()
    return "connecting europe facility" in token
...
def select_cef_candidate(taxonomy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in taxonomy.get("records") or []:
        if row.get("programme_family_normalized") != "CEF":
            continue
    ...
    if not candidates:
        raise ValueError("programme taxonomy contains no CEF candidate")
...
def collect_exact(
    ...
    *,
    ...
    for row in primary:
        ...
        if not programme_label or not _is_cef_label(programme_label):
            raise ValueError(f"exact record is not proven to belong to CEF: {programme_ref!r} {programme_label!r}")
    ...
    if len(signatures) != 1:
        raise ExactCefConflict(f"conflicting exact CEF records for {reference}: {len(signatures)} material variants")
    ...
    if readback.get("verified") is not True:
        raise ValueError(f"exact CEF topic authority readback failed for {reference}: {readback.get('error')}")
    ...
    if len(records) != 1:
        raise ValueError(f"exact CEF normalizer returned {len(records)} records for {reference}")
    normalized = records[0]
    if normalized.get("authority_url_verified") is not True:
        raise ValueError("exact CEF authority verification was lost during normalization")
    ...
    if output_dir:
        ...
        (output_dir / "ft-cef-search-response.json").write_bytes(search_raw)
        (output_dir / "ft-cef-facet-response.json").write_bytes(facet_raw)
        (output_dir / "ft-cef-exact-evidence.json").write_text(
...
def validate_evidence(evidence: Mapping[str, Any]) -> None:
    if evidence.get("schema") != SCHEMA or evidence.get("parser_version") != PARSER_VERSION:
        raise ValueError("CEF exact evidence schema/parser drift")
    ...
    if evidence.get("source_family") != SOURCE_FAMILY or evidence.get("programme_family") != PROGRAMME_FAMILY:
        raise ValueError("CEF exact evidence family drift")
    if evidence.get("authority_class") != AUTHORITY_CLASS:
        raise ValueError("CEF exact evidence authority class drift")
    if evidence.get("authority_url") != ft.topic_url(reference) or evidence.get("authority_url_verified") is not True:
        raise ValueError("CEF exact evidence lacks verified exact topic authority")
    ...
    if readback.get("verified") is not True or readback.get("url") != evidence.get("authority_url"):
        raise ValueError("CEF exact readback binding invalid")
    label = str(evidence.get("programme_label_official") or "")
    if not _is_cef_label(label):
        raise ValueError("CEF exact evidence lost official programme proof")
    if evidence.get("candidate_state") not in {"OPEN_CALL", "FORTHCOMING_CALL", "CLOSED_CALL", "UNKNOWN"}:
        raise ValueError("CEF exact candidate state unsupported")
    if not evidence.get("status_label"):
        raise ValueError("CEF exact evidence lacks resolved official status label")
    ...
    if not isinstance(semantics, dict) or sha256_json(semantics) != evidence.get("exact_semantic_fingerprint"):
        raise ValueError("CEF exact semantic fingerprint mismatch")
    for key in MATERIAL_FLAGS:
        if evidence.get(key) is not False:
            raise ValueError(f"CEF exact evidence attempted authorization: {key}")
    if evidence.get("publication_effect") != "NONE" or evidence.get("canonical_corpus_mutation") is not False:
        raise ValueError("CEF exact evidence crossed publication boundary")
    if evidence.get("semantic_reconciliation_required") is not True or evidence.get("field_scoped_material_admission_required") is not True:
        raise ValueError("CEF exact evidence skipped downstream gates")
    for receipt_key in ("search_receipt", "facet_receipt"):
        ...
        if not re.fullmatch(r"[0-9a-f]{64}", str(receipt.get("sha256") or "")):
            raise ValueError(f"CEF exact evidence missing immutable {receipt_key}")
...
def main() -> int:
    ...
    parser.add_argument("--run-id", default="cef-ft-exact-live")
    ...
    if args.taxonomy:
        ...
        source_candidate = select_cef_candidate(taxonomy)
        if reference and validate_reference(reference) != source_candidate["identifier"]:
            raise ValueError("explicit CEF reference does not match deterministic taxonomy handoff")
