#!/usr/bin/env python3
"""Semantic reconciliation for exact EEA Civil Society Fund Romania Call #7 evidence."""
...
SCHEMA = "PARTENER_EU_EEA_CSF_RO_CALL7_RECONCILIATION_V1"
PARSER_VERSION = "EEA_CSF_RO_CALL7_RECONCILE_V1"
MATERIAL_FLAGS = (
    "material_fact_use",
...
def reconcile(current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if current.get("schema") != EXACT_SCHEMA:
        raise ValueError("current evidence is not EEA CSF Call #7 exact evidence")
    ...
    if previous is not None:
        if previous.get("schema") != EXACT_SCHEMA:
            raise ValueError("previous evidence is not EEA CSF Call #7 exact evidence")
        ...
        if previous.get("official_call_identifier") != OFFICIAL_CALL_IDENTIFIER:
            raise ValueError("EEA CSF previous evidence lost Call #7 identifier")
    ...
    if not current_healthy:
        ...
        state = "NO_CHANGE" if not changes else "EEA_CSF_CALL7_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
...
def validate_receipt(
    ...
    if receipt.get("schema") != SCHEMA or receipt.get("parser_version") != PARSER_VERSION:
        raise ValueError("EEA CSF Call #7 reconciliation schema/parser drift")
    ...
    if not current_healthy:
        ...
        expected = (
            ...
            else "EEA_CSF_CALL7_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
        )
