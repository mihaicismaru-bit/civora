#!/usr/bin/env python3
"""Semantic reconciliation for exact current CERV Funding & Tenders evidence."""
...
from eu_direct_cerv_ft_exact import SCHEMA as EXACT_SCHEMA, canonical_json, validate_evidence

SCHEMA = "PARTENER_EU_CERV_FT_RECONCILIATION_V1"
PARSER_VERSION = "EU_DIRECT_CERV_FT_RECONCILE_V1"
MATERIAL_FLAGS = (
...
def _validated_semantics(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(semantics, dict) or sha256_json(semantics) != evidence.get("exact_semantic_fingerprint"):
        raise ValueError("exact CERV semantic fingerprint tampered")
...
def reconcile(current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if current.get("schema") != EXACT_SCHEMA:
        raise ValueError("current evidence is not CERV exact evidence")
    ...
    if previous is None:
        ...
        if previous.get("schema") != EXACT_SCHEMA:
            raise ValueError("previous evidence is not CERV exact evidence")
        previous_semantics = _validated_semantics(previous)
        if previous.get("reference") != current.get("reference"):
            raise ValueError("CERV reconciliation identity mismatch")
        if parse_time(str(previous.get("fetched_at"))) > parse_time(str(current.get("fetched_at"))):
            raise ValueError("previous CERV evidence is newer than current evidence")
        for key in sorted(set(previous_semantics) | set(current_semantics)):
            if previous_semantics.get(key) != current_semantics.get(key):
                changes.append({"field": key, "before": previous_semantics.get(key), "after": current_semantics.get(key)})
        state = "NO_CHANGE" if not changes else "CERV_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
...
def validate_receipt(
    ...
    if receipt.get("schema") != SCHEMA or receipt.get("parser_version") != PARSER_VERSION:
        raise ValueError("CERV reconciliation schema/parser drift")
    ...
    if receipt.get("reference") != current.get("reference") or receipt.get("current_evidence_sha256") != sha256_json(current):
        raise ValueError("CERV reconciliation current binding failed")
    if previous is None:
        if receipt.get("reconciliation_state") != "BASELINE_CAPTURED_NON_AUTHORIZING":
            raise ValueError("CERV baseline reconciliation invalid")
        if receipt.get("previous_evidence_sha256") is not None:
            raise ValueError("CERV baseline unexpectedly bound previous evidence")
    else:
        validate_evidence(previous)
        if receipt.get("previous_evidence_sha256") != sha256_json(previous):
            raise ValueError("CERV previous evidence hash mismatch")
        expected = (
            "NO_CHANGE"
            if current.get("exact_semantic_fingerprint") == previous.get("exact_semantic_fingerprint")
            else "CERV_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
        )
        if receipt.get("reconciliation_state") != expected:
            raise ValueError("CERV reconciliation state disagrees with fingerprints")
    ready = (
        current.get("candidate_state") == "OPEN_CALL"
        and current.get("authority_url_verified") is True
    ...
    if receipt.get("material_admission_ready_for_downstream_review") is not bool(ready):
        raise ValueError("CERV downstream-review gate drift")
    for key in MATERIAL_FLAGS:
        if receipt.get(key) is not False:
            raise ValueError(f"CERV reconciliation attempted authorization: {key}")
    if receipt.get("publication_effect") != "NONE" or receipt.get("canonical_corpus_mutation") is not False:
        raise ValueError("CERV reconciliation crossed publication boundary")
