#!/usr/bin/env python3
"""Resolve one bounded Digital Europe exact-handoff target without inferring material status.

The programme-wide F&T sample is explicitly non-exhaustive. If it contains a
Digital Europe candidate, that current discovery pointer wins. If the bounded
sample omits the family, a validated previous exact Digital Europe receipt may
be used only as a pointer for a fresh exact authority recheck. If neither
exists, the family is skipped for this run with a non-authorizing omission
receipt.
"""
...
from eu_direct_digital_ft_exact import select_digital_candidate, validate_evidence
...
def resolve_handoff(
    ...
    *,
    ...
    try:
        current_candidate = select_digital_candidate(taxonomy)
...
def validate_state(state: Mapping[str, Any]) -> None:
    ...
    if mode == OMITTED_RECHECK_MODE:
        if state.get("current_taxonomy_candidate") is not False or state.get("previous_evidence_available") is not True:
            raise ValueError("Digital Europe omission recheck lacks validated previous pointer")
        if state.get("previous_same_identity") is not True:
            raise ValueError("Digital Europe omission recheck previous identity mismatch")
    if mode == OMITTED_SKIP_MODE:
        if state.get("current_taxonomy_candidate") is not False or state.get("previous_evidence_available") is not False:
            raise ValueError("Digital Europe omission skip unexpectedly has a candidate")
