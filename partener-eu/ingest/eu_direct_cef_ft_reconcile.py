#!/usr/bin/env python3
"""Semantic reconciliation for exact current CEF Funding & Tenders evidence."""
...
from eu_direct_cef_ft_exact import SCHEMA as EXACT_SCHEMA, canonical_json, validate_evidence

SCHEMA = "PARTENER_EU_CEF_FT_RECONCILIATION_V1"
PARSER_VERSION = "EU_DIRECT_CEF_FT_RECONCILE_V1"
MATERIAL_FLAGS = (
    "material_fact_use",
...
def _validated_semantics(evidence: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(semantics, dict):
        raise ValueError("exact CEF semantics missing")
    if sha256_json(semantics) != evidence.get("exact_semantic_fingerprint"):
        raise ValueError("exact CEF semantic fingerprint tampered")
...
def reconcile(current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if current.get("schema") != EXACT_SCHEMA:
        raise ValueError("current evidence is not CEF exact evidence")
    ...
    if previous is None:
        ...
        if previous.get("schema") != EXACT_SCHEMA:
            raise ValueError("previous evidence is not CEF exact evidence")
        previous_semantics = _validated_semantics(previous)
        if previous.get("reference") != current.get("reference"):
            raise ValueError("CEF reconciliation identity mismatch")
        if parse_time(str(previous.get("fetched_at"))) > parse_time(str(current.get("fetched_at"))):
            raise ValueError("previous CEF evidence is newer than current evidence")
        ...
        state = "NO_CHANGE" if not changes else "CEF_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
...
def validate_receipt(receipt: Mapping[str, Any], *, current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> None:
    if receipt.get("schema") != SCHEMA or receipt.get("parser_version") != PARSER_VERSION:
        raise ValueError("CEF reconciliation schema/parser drift")
    validate_evidence(current)
    if receipt.get("reference") != current.get("reference"):
        raise ValueError("CEF reconciliation current identity binding failed")
    if receipt.get("current_evidence_sha256") != sha256_json(current):
        raise ValueError("CEF reconciliation current evidence hash mismatch")
    if receipt.get("current_exact_semantic_fingerprint") != current.get("exact_semantic_fingerprint"):
        raise ValueError("CEF reconciliation current semantic binding failed")
    if previous is None:
        if receipt.get("reconciliation_state") != "BASELINE_CAPTURED_NON_AUTHORIZING":
            raise ValueError("CEF baseline reconciliation state invalid")
        if receipt.get("previous_evidence_sha256") is not None:
            raise ValueError("CEF baseline unexpectedly bound previous evidence")
    else:
        validate_evidence(previous)
        if receipt.get("previous_evidence_sha256") != sha256_json(previous):
            raise ValueError("CEF reconciliation previous evidence hash mismatch")
        expected = "NO_CHANGE" if current.get("exact_semantic_fingerprint") == previous.get("exact_semantic_fingerprint") else "CEF_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
        if receipt.get("reconciliation_state") != expected:
            raise ValueError("CEF reconciliation state disagrees with semantic fingerprints")
    ...
    if receipt.get("material_admission_ready_for_downstream_review") is not bool(ready):
        raise ValueError("CEF reconciliation downstream-review gate drift")
    if receipt.get("semantic_reconciliation_passed") is not True or receipt.get("field_scoped_material_admission_required") is not True:
        raise ValueError("CEF reconciliation skipped mandatory gate")
    for key in MATERIAL_FLAGS:
        if receipt.get(key) is not False:
            raise ValueError(f"CEF reconciliation attempted authorization: {key}")
    if receipt.get("publication_effect") != "NONE" or receipt.get("canonical_corpus_mutation") is not False:
        raise ValueError("CEF reconciliation crossed publication boundary")
