#!/usr/bin/env python3
"""Bounded semantic reconciliation common for EEA CSF Romania Calls #1-#3."""
...
from eea_civil_society_fund_exact_common import (
    ...
    canonical_json,
    get_spec,
    validate_evidence as validate_exact_evidence,
)


def schema(call_id: str) -> str:
    return f"PARTENER_EU_EEA_CSF_RO_CALL{call_id}_RECONCILIATION_V1"


def parser_version(call_id: str) -> str:
    return f"EEA_CSF_RO_CALL{call_id}_RECONCILE_V1"
...
def _validated_semantics(call_id: str, evidence: Mapping[str, Any]) -> dict[str, Any]:
    validate_exact_evidence(call_id, evidence)
...
def _healthy(evidence: Mapping[str, Any] | None) -> bool:
    return bool(evidence is not None and evidence.get("source_health_state") == "HEALTHY" and evidence.get("lkg_required") is False)


def reconcile(call_id: str, current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    spec = get_spec(call_id)
    if current.get("schema") != spec.schema:
        raise ValueError(f"current evidence is not EEA CSF Call #{call_id} exact evidence")
    current_semantics = _validated_semantics(call_id, current)
    previous_semantics: dict[str, Any] | None = None

    if previous is not None:
        if previous.get("schema") != spec.schema:
            raise ValueError(f"previous evidence is not EEA CSF Call #{call_id} exact evidence")
        previous_semantics = _validated_semantics(call_id, previous)
        if previous.get("identity_key") != current.get("identity_key"):
            raise ValueError("EEA CSF exact reconciliation identity mismatch")
        if previous.get("official_call_identifier") != call_id:
            raise ValueError(f"EEA CSF previous evidence lost Call #{call_id} identifier")
    ...
    if not current_healthy:
        ...
        state = "NO_CHANGE" if not changes else f"EEA_CSF_CALL{call_id}_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
    ...
    exact_current_status_proven = bool(
        ...
        and current.get("official_call_identifier") == call_id
    ...
    receipt: dict[str, Any] = {
        "schema": schema(call_id),
        "parser_version": parser_version(call_id),
        "source_family": SOURCE_FAMILY,
        "programme_family": PROGRAMME_FAMILY,
        "programme_id": PROGRAMME_ID,
        "authority_class": AUTHORITY_CLASS,
        "official_call_identifier": call_id,
    ...
    for key in MATERIAL_FLAGS:
        receipt[key] = False
    validate_receipt(call_id, receipt, current=current, previous=previous)
    return receipt
...
def validate_receipt(
    ...
    *,
    ...
    spec = get_spec(call_id)
    if receipt.get("schema") != schema(call_id) or receipt.get("parser_version") != parser_version(call_id):
        raise ValueError(f"EEA CSF Call #{call_id} reconciliation schema/parser drift")
    validate_exact_evidence(call_id, current)
    ...
    if receipt.get("programme_id") != PROGRAMME_ID or receipt.get("official_call_identifier") != call_id:
    ...
    previous_healthy = _healthy(previous)
    if previous is not None:
        validate_exact_evidence(call_id, previous)
        if previous.get("identity_key") != current.get("identity_key"):
    ...
    if not current_healthy:
        ...
        expected = "NO_CHANGE" if current.get("exact_semantic_fingerprint") == previous.get("exact_semantic_fingerprint") else f"EEA_CSF_CALL{call_id}_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
    ...
    if current.get("schema") != spec.schema:
        raise ValueError("EEA CSF reconciliation current schema drift")
...
def main_for(call_id: str) -> int:
    ...
    receipt = reconcile(call_id, current, previous)
