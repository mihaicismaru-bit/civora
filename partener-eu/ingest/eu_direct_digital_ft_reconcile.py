...
PARSER_VERSION = "EU_DIRECT_DIGITAL_FT_RECONCILE_V1"
MATERIAL_FLAGS = (
    "material_fact_use",
...
def validate_receipt(
    ...
    if previous is None:
        if receipt.get("reconciliation_state") != "BASELINE_CAPTURED_NON_AUTHORIZING":
            raise ValueError("Digital Europe baseline reconciliation state invalid")
        if receipt.get("previous_evidence_sha256") is not None:
            raise ValueError("Digital Europe baseline unexpectedly bound previous evidence")
    else:
    ...
    if receipt.get("semantic_reconciliation_passed") is not True or receipt.get("field_scoped_material_admission_required") is not True:
        raise ValueError("Digital Europe reconciliation skipped mandatory gate")
    for key in MATERIAL_FLAGS:
