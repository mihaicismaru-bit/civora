#!/usr/bin/env python3
"""Exact-current, non-authorizing evidence for EEA Civil Society Fund Romania Call #7.

This bounded adapter binds the official eeagrants.org calls index to the exact
Call #7 detail page. The exact page exposes an official call number and current
...
SCHEMA = "PARTENER_EU_EEA_CSF_RO_CALL7_EXACT_EVIDENCE_V1"
PARSER_VERSION = "EEA_CSF_RO_CALL7_EXACT_V1"
...
EXACT_URL = (
    ...
    "call-7-organisational-development-experienced-csos-and-federations"
)
EXPECTED_SLUG = "call-7-organisational-development-experienced-csos-and-federations"
EXPECTED_TITLE_MARKERS = (
    "Apel #7 Dezvoltare organizațională pentru OSC-uri cu experiență și federații",
    "Call #7 Organisational Development for Experienced CSOs and Federations",
)
...
class ExactCSFCall7Error(ValueError):
    pass
...
def _validate_url(url: str, *, exact: bool = False) -> str:
    ...
    if parsed.scheme != "https" or (parsed.hostname or "").casefold() != "eeagrants.org":
        raise ExactCSFCall7Error(f"unexpected EEA CSF authority URL: {url!r}")
    path = parsed.path.rstrip("/")
    prefix = "/ro/eea-civil-society-fund-romania/calls"
    if not path.startswith(prefix):
        raise ExactCSFCall7Error(f"EEA CSF authority path outside bounded allowlist: {parsed.path!r}")
    if exact and path.split("/")[-1] != EXPECTED_SLUG:
        raise ExactCSFCall7Error("bounded EEA CSF exact adapter accepts only official Call #7 detail")
    return url
...
def _http_fetch(url: str, *, timeout: float = 25.0) -> tuple[bytes, int, str, str]:
    ...
    with urlopen(req, timeout=timeout) as response:
        ...
        if len(raw) > MAX_BYTES:
            raise ExactCSFCall7Error("EEA CSF exact source exceeded bounded acquisition limit")
...
def _detail_semantics(raw: bytes) -> dict[str, Any]:
    ...
    if not any(_fold(marker) in folded for marker in EXPECTED_TITLE_MARKERS):
        raise ExactCSFCall7Error("exact EEA CSF detail lost Call #7 title")
    if "civil society fund" not in folded:
        raise ExactCSFCall7Error("exact EEA CSF detail lost programme marker")
    if not re.search(r"numarul apelului de proiecte\s+7\b", folded) and not re.search(r"call number\s+7\b", folded):
        raise ExactCSFCall7Error("exact EEA CSF detail lost official Call number 7")
    ...
    amount = "EUR 3,000,000" if (
        re.search(r"suma disponibila\s+€?\s*3,000,000", folded)
        or re.search(r"amount available\s+€?\s*3,000,000", folded)
    ) else None
    minimum = "EUR 200,001" if (
        re.search(r"valoarea minima a grantului\s+€?\s*200,001", folded)
        or re.search(r"grant amount from\s+€?\s*200,001", folded)
    ) else None
    maximum = "EUR 350,000" if (
        re.search(r"valoarea maxima a grantului\s+€?\s*350,000", folded)
        or re.search(r"grant amount to\s+€?\s*350,000", folded)
    ) else None
...
def collect_exact(
    *,
    ...
    if index_receipt["health_state"] == "HEALTHY" and not discovery_link_verified:
        index_receipt = {
            **index_receipt,
            ...
            "lkg_required": True,
            "error": "official EEA CSF call index lost exact Call #7 binding",
        }
    ...
    if detail_raw is not None and detail_receipt["health_state"] == "HEALTHY":
        ...
        except ExactCSFCall7Error as exc:
            detail_receipt = {
                **detail_receipt,
                "health_state": "DEGRADED_MARKER_MISMATCH",
                "lkg_required": True,
    ...
    if output_dir:
        ...
        if detail_raw is not None:
            (output_dir / "eea-csf-ro-call7-detail.html").write_bytes(detail_raw)
        (output_dir / "eea-csf-ro-call7-exact-evidence.json").write_text(
...
def validate_evidence(evidence: Mapping[str, Any]) -> None:
    if evidence.get("schema") != SCHEMA or evidence.get("parser_version") != PARSER_VERSION:
        raise ExactCSFCall7Error("EEA CSF Call #7 exact schema/parser drift")
    if evidence.get("source_family") != SOURCE_FAMILY or evidence.get("programme_family") != PROGRAMME_FAMILY:
        raise ExactCSFCall7Error("EEA CSF Call #7 family drift")
    if evidence.get("programme_id") != PROGRAMME_ID:
        raise ExactCSFCall7Error("EEA CSF programme identity drift")
    if evidence.get("authority_class") != AUTHORITY_CLASS or evidence.get("observation_state") != OBSERVATION_STATE:
        raise ExactCSFCall7Error("EEA CSF authority/observation drift")
    ...
    if evidence.get("official_call_identifier") != OFFICIAL_CALL_IDENTIFIER or evidence.get("call_identifier_kind") != CALL_IDENTIFIER_KIND:
        raise ExactCSFCall7Error("EEA CSF official Call #7 identifier drift")
    identity_basis = {
        "programme_id": PROGRAMME_ID,
        "official_call_identifier": OFFICIAL_CALL_IDENTIFIER,
        "call_identifier_kind": CALL_IDENTIFIER_KIND,
    ...
    if evidence.get("identity_key") != sha256_json(identity_basis):
        raise ExactCSFCall7Error("EEA CSF exact identity fingerprint mismatch")
    ...
    if not isinstance(semantics, Mapping) or sha256_json(dict(semantics)) != evidence.get("exact_semantic_fingerprint"):
        raise ExactCSFCall7Error("EEA CSF exact semantic fingerprint mismatch")
    if semantics.get("official_call_identifier") != OFFICIAL_CALL_IDENTIFIER:
        raise ExactCSFCall7Error("EEA CSF exact semantics lost official Call #7 identifier")
    if evidence.get("candidate_state") not in {"OPEN_CALL", "CLOSED_CALL", "UNKNOWN"}:
        raise ExactCSFCall7Error("EEA CSF exact candidate state unsupported")
    if evidence.get("source_health_state") == "HEALTHY":
        if evidence.get("lkg_required") is not False or evidence.get("discovery_link_verified") is not True:
            raise ExactCSFCall7Error("healthy EEA CSF exact chain lost discovery/detail binding")
        receipts = evidence.get("source_receipts") or {}
        if set(receipts) != {"official_calls_index_discovery", "official_exact_call_detail"}:
            raise ExactCSFCall7Error("EEA CSF source receipt set drift")
        for receipt in receipts.values():
            if receipt.get("health_state") != "HEALTHY" or receipt.get("http_status") != 200:
                raise ExactCSFCall7Error("healthy EEA CSF aggregate contains degraded receipt")
            if not re.fullmatch(r"[0-9a-f]{64}", str(receipt.get("raw_sha256") or "")):
                raise ExactCSFCall7Error("healthy EEA CSF receipt lost raw SHA-256")
    elif evidence.get("lkg_required") is not True:
        raise ExactCSFCall7Error("degraded EEA CSF exact chain did not require LKG/reference handling")
    ...
    if "semantic_reconciliation" not in missing or "field_scoped_material_admission" not in missing:
        raise ExactCSFCall7Error("EEA CSF exact evidence weakened downstream gates")
    for key in MATERIAL_FLAGS:
        if evidence.get(key) is not False:
            raise ExactCSFCall7Error(f"EEA CSF exact evidence attempted authorization: {key}")
    if evidence.get("semantic_reconciliation_required") is not True or evidence.get("field_scoped_material_admission_required") is not True:
        raise ExactCSFCall7Error("EEA CSF exact evidence skipped mandatory gates")
    if evidence.get("publication_effect") != "NONE" or evidence.get("canonical_corpus_mutation") is not False:
        raise ExactCSFCall7Error("EEA CSF exact evidence crossed publication boundary")
