#!/usr/bin/env python3
"""Semantic reconciliation for exact EEA Civil Society Fund Romania Call #4 evidence."""
...
from eea_civil_society_fund_call4_exact import (
    AUTHORITY_CLASS,
...
SCHEMA = "PARTENER_EU_EEA_CSF_RO_CALL4_RECONCILIATION_V1"
PARSER_VERSION = "EEA_CSF_RO_CALL4_RECONCILE_V1"
MATERIAL_FLAGS = (
    "material_fact_use",
...
def reconcile(current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if current.get("schema") != EXACT_SCHEMA:
        raise ValueError("current evidence is not EEA CSF Call #4 exact evidence")
    ...
    if previous is not None:
        if previous.get("schema") != EXACT_SCHEMA:
            raise ValueError("previous evidence is not EEA CSF Call #4 exact evidence")
        ...
        if previous.get("official_call_identifier") != OFFICIAL_CALL_IDENTIFIER:
            raise ValueError("EEA CSF previous evidence lost Call #4 identifier")
    ...
    if not current_healthy:
        ...
        state = "NO_CHANGE" if not changes else "EEA_CSF_CALL4_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
...
def validate_receipt(
    ...
    if receipt.get("schema") != SCHEMA or receipt.get("parser_version") != PARSER_VERSION:
        raise ValueError("EEA CSF Call #4 reconciliation schema/parser drift")
    ...
    if not current_healthy:
        ...
        expected = (
            ...
            else "EEA_CSF_CALL4_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
        )
