...
PARSER_VERSION = "EU_DIRECT_SMP_FT_RECONCILE_V1_1"
MATERIAL_FLAGS = (
...
def _evidence_usable(evidence: Mapping[str, Any]) -> bool:
    ...
    if value not in {True, False}:
        raise ValueError("Single Market Programme evidence usability state invalid")
    return bool(value)
...
def _validate_previous_identity_and_time(current: Mapping[str, Any], previous: Mapping[str, Any]) -> None:
    if previous.get("schema") != EXACT_SCHEMA:
        raise ValueError("previous evidence is not Single Market Programme exact evidence")
    validate_evidence(previous)
    if previous.get("reference") != current.get("reference"):
        raise ValueError("Single Market Programme reconciliation identity mismatch")
    if parse_time(str(previous.get("fetched_at"))) > parse_time(str(current.get("fetched_at"))):
        raise ValueError("previous Single Market Programme evidence is newer than current evidence")


def reconcile(current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> dict[str, Any]:
    if current.get("schema") != EXACT_SCHEMA:
        raise ValueError("current evidence is not Single Market Programme exact evidence")
    ...
    receipt: dict[str, Any] = {
        ...
        "lkg_reference_required": not current_usable,
...
def validate_receipt(receipt: Mapping[str, Any], *, current: Mapping[str, Any], previous: Mapping[str, Any] | None = None) -> None:
    if receipt.get("schema") != SCHEMA or receipt.get("parser_version") != PARSER_VERSION:
        raise ValueError("Single Market Programme reconciliation schema/parser drift")
    validate_evidence(current)
    current_usable = _evidence_usable(current)
    if receipt.get("reference") != current.get("reference") or receipt.get("current_evidence_sha256") != sha256_json(current):
        raise ValueError("Single Market Programme reconciliation current binding failed")
    if receipt.get("current_exact_semantic_fingerprint") != current.get("exact_semantic_fingerprint"):
        raise ValueError("Single Market Programme reconciliation current semantic binding failed")
    if receipt.get("current_evidence_usable_for_reconciliation") is not current_usable:
        raise ValueError("Single Market Programme reconciliation usability binding failed")
    if previous is not None:
        _validate_previous_identity_and_time(current, previous)
        if receipt.get("previous_evidence_sha256") != sha256_json(previous):
            raise ValueError("Single Market Programme previous evidence hash mismatch")
    elif receipt.get("previous_evidence_sha256") is not None:
        raise ValueError("Single Market Programme reconciliation unexpectedly bound previous evidence")

    if not current_usable:
        if receipt.get("reconciliation_state") != DEGRADED_STATE:
            raise ValueError("Single Market Programme degraded reconciliation state invalid")
        if receipt.get("semantic_reconciliation_passed") is not False:
            raise ValueError("Single Market Programme degraded current evidence pretended semantic reconciliation")
        if receipt.get("semantic_change_count") != 0 or receipt.get("semantic_changes") != []:
            raise ValueError("Single Market Programme degraded current evidence invented semantic change")
        if receipt.get("lkg_reference_required") is not True:
            raise ValueError("Single Market Programme degraded current evidence lost LKG requirement")
        if receipt.get("lkg_reference_available") is not (previous is not None):
            raise ValueError("Single Market Programme degraded current evidence LKG availability drift")
        if receipt.get("material_admission_ready_for_downstream_review") is not False:
            raise ValueError("Single Market Programme degraded current evidence became review-ready")
    else:
        if previous is None:
            ...
            expected = (
                ...
                else "SMP_EXACT_TOPIC_SEMANTIC_CHANGE_RECONCILED_NON_AUTHORIZING"
            )
        if receipt.get("reconciliation_state") != expected:
            raise ValueError("Single Market Programme reconciliation state disagrees with fingerprints")
        if receipt.get("semantic_reconciliation_passed") is not True:
            raise ValueError("Single Market Programme healthy evidence failed semantic reconciliation")
        if receipt.get("lkg_reference_required") is not False:
            raise ValueError("Single Market Programme healthy evidence unexpectedly requires LKG")
        ...
        if receipt.get("material_admission_ready_for_downstream_review") is not bool(ready):
            raise ValueError("Single Market Programme downstream-review gate drift")

    if receipt.get("lkg_reference_is_current_truth") is not False:
        raise ValueError("Single Market Programme reconciliation promoted LKG to current truth")
    if receipt.get("field_scoped_material_admission_required") is not True:
        raise ValueError("Single Market Programme reconciliation skipped mandatory admission gate")
    for key in MATERIAL_FLAGS:
        if receipt.get(key) is not False:
            raise ValueError(f"Single Market Programme reconciliation attempted authorization: {key}")
    if receipt.get("publication_effect") != "NONE" or receipt.get("canonical_corpus_mutation") is not False:
        raise ValueError("Single Market Programme reconciliation crossed publication boundary")
