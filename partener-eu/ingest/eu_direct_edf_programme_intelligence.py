...
def validate_registry(registry: Mapping[str, Any]) -> None:
    if registry.get("schema") != REGISTRY_SCHEMA:
        raise ValueError("EDF registry schema drift")
    if registry.get("source_family") != "EU_DIRECT" or registry.get("programme_family") != "European Defence Fund":
        raise ValueError("EDF family drift")
    policy = registry.get("policy") or {}
    for flag in MATERIAL_FLAGS:
        if policy.get(flag) is not False:
            raise ValueError(f"EDF registry became authorizing: {flag}")
    for key in (
        ...
        if policy.get(key) is not True:
            raise ValueError(f"EDF policy weakened: {key}")
    sources = registry.get("sources") or []
    if len(sources) != 3:
        raise ValueError("EDF registry source inventory drift")
    allowed_states = {"PROGRAMME_INTELLIGENCE", "PROGRAMMING_PIPELINE", "CALL_INDEX_DISCOVERY"}
    for row in sources:
        ...
        if parsed.scheme != "https" or (parsed.hostname or "").casefold() not in ALLOWED_HOSTS:
            raise ValueError("EDF authority URL drift")
        if row.get("observation_state") not in allowed_states:
            raise ValueError("EDF observation state became call state")
        if not row.get("required_markers"):
            raise ValueError("EDF source markers missing")
    fit = registry.get("applicant_fit") or {}
    if fit.get("fit_is_not_eligibility") is not True:
        raise ValueError("EDF fit became eligibility")
...
def fetch(url: str, timeout: float = 30.0) -> tuple[bytes, dict[str, Any]]:
    ...
    with urllib.request.urlopen(req, timeout=timeout) as response:
        ...
        if len(raw) > 3_000_000:
            raise ValueError("EDF source exceeds 3 MB")
        return raw, {
            "requested_url": url,
