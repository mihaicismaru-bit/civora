#!/usr/bin/env python3
"""Exact-current, non-authorizing evidence for EEA Civil Society Fund Romania Call #6.

This bounded adapter binds the official eeagrants.org calls index to the exact
Call #6 detail page. Exact status/deadline/budget fields remain candidate
evidence only; semantic reconciliation and field-scoped material admission are
mandatory before any reader-facing use.
"""
...
SCHEMA = "PARTENER_EU_EEA_CSF_RO_CALL6_EXACT_EVIDENCE_V1"
PARSER_VERSION = "EEA_CSF_RO_CALL6_EXACT_V1"
...
EXACT_URL = (
    ...
    "call-6-protecting-human-rights-through-climate-and-environmental-actions"
)
EXPECTED_SLUG = "call-6-protecting-human-rights-through-climate-and-environmental-actions"
EXPECTED_TITLE_MARKERS = (
    "Apel #6 Protejarea drepturilor omului prin acțiuni climatice și de mediu",
    "Call #6 Protecting Human Rights through Climate and Environmental Actions",
)
...
class ExactCSFCall6Error(ValueError):
    pass
...
def _validate_url(url: str, *, exact: bool = False) -> str:
    ...
    if parsed.scheme != "https" or (parsed.hostname or "").casefold() != "eeagrants.org":
        raise ExactCSFCall6Error(f"unexpected EEA CSF authority URL: {url!r}")
    path = parsed.path.rstrip("/")
    prefix = "/ro/eea-civil-society-fund-romania/calls"
    if not path.startswith(prefix):
        raise ExactCSFCall6Error(f"EEA CSF authority path outside bounded allowlist: {parsed.path!r}")
    if exact and path.split("/")[-1] != EXPECTED_SLUG:
        raise ExactCSFCall6Error("bounded EEA CSF exact adapter accepts only official Call #6 detail")
    return url
...
def _http_fetch(url: str, *, timeout: float = 25.0) -> tuple[bytes, int, str, str]:
    ...
    with urlopen(req, timeout=timeout) as response:
        ...
        if len(raw) > MAX_BYTES:
            raise ExactCSFCall6Error("EEA CSF exact source exceeded bounded acquisition limit")
...
def _detail_semantics(raw: bytes) -> dict[str, Any]:
    ...
    if not any(_fold(marker) in folded for marker in EXPECTED_TITLE_MARKERS):
        raise ExactCSFCall6Error("exact EEA CSF detail lost Call #6 title")
    if "civil society fund" not in folded:
        raise ExactCSFCall6Error("exact EEA CSF detail lost programme marker")
    if not re.search(r"numarul apelului de proiecte\s+6\b", folded) and not re.search(r"call number\s+6\b", folded):
        raise ExactCSFCall6Error("exact EEA CSF detail lost official Call number 6")
    ...
    amount = "EUR 6,000,000" if (
        re.search(r"suma disponibila\s+€?\s*6,000,000", folded)
        or re.search(r"amount available\s+€?\s*6,000,000", folded)
    ) else None
    minimum = "EUR 15,000" if (
        re.search(r"valoarea minima a grantului\s+€?\s*15,000", folded)
        or re.search(r"grant amount from\s+€?\s*15,000", folded)
    ) else None
...
def collect_exact(
    *,
    ...
    if index_receipt["health_state"] == "HEALTHY" and not discovery_link_verified:
        index_receipt = {
            **index_receipt,
            ...
            "lkg_required": True,
            "error": "official EEA CSF call index lost exact Call #6 binding",
        }
    ...
    if detail_raw is not None and detail_receipt["health_state"] == "HEALTHY":
        ...
        except ExactCSFCall6Error as exc:
            detail_receipt = {
                **detail_receipt,
                "health_state": "DEGRADED_MARKER_MISMATCH",
                "lkg_required": True,
    ...
    if output_dir:
        ...
        if detail_raw is not None:
            (output_dir / "eea-csf-ro-call6-detail.html").write_bytes(detail_raw)
        (output_dir / "eea-csf-ro-call6-exact-evidence.json").write_text(
...
def validate_evidence(evidence: Mapping[str, Any]) -> None:
    if evidence.get("schema") != SCHEMA or evidence.get("parser_version") != PARSER_VERSION:
        raise ExactCSFCall6Error("EEA CSF Call #6 exact schema/parser drift")
    if evidence.get("source_family") != SOURCE_FAMILY or evidence.get("programme_family") != PROGRAMME_FAMILY:
        raise ExactCSFCall6Error("EEA CSF Call #6 family drift")
    if evidence.get("programme_id") != PROGRAMME_ID:
        raise ExactCSFCall6Error("EEA CSF programme identity drift")
    if evidence.get("authority_class") != AUTHORITY_CLASS or evidence.get("observation_state") != OBSERVATION_STATE:
        raise ExactCSFCall6Error("EEA CSF authority/observation drift")
    ...
    if evidence.get("official_call_identifier") != OFFICIAL_CALL_IDENTIFIER or evidence.get("call_identifier_kind") != CALL_IDENTIFIER_KIND:
        raise ExactCSFCall6Error("EEA CSF official Call #6 identifier drift")
    identity_basis = {
        "programme_id": PROGRAMME_ID,
        "official_call_identifier": OFFICIAL_CALL_IDENTIFIER,
        "call_identifier_kind": CALL_IDENTIFIER_KIND,
    ...
    if evidence.get("identity_key") != sha256_json(identity_basis):
        raise ExactCSFCall6Error("EEA CSF exact identity fingerprint mismatch")
    ...
    if not isinstance(semantics, Mapping) or sha256_json(dict(semantics)) != evidence.get("exact_semantic_fingerprint"):
        raise ExactCSFCall6Error("EEA CSF exact semantic fingerprint mismatch")
    if semantics.get("official_call_identifier") != OFFICIAL_CALL_IDENTIFIER:
        raise ExactCSFCall6Error("EEA CSF exact semantics lost official Call #6 identifier")
    if evidence.get("candidate_state") not in {"OPEN_CALL", "CLOSED_CALL", "UNKNOWN"}:
        raise ExactCSFCall6Error("EEA CSF exact candidate state unsupported")
    if evidence.get("source_health_state") == "HEALTHY":
        if evidence.get("lkg_required") is not False or evidence.get("discovery_link_verified") is not True:
            raise ExactCSFCall6Error("healthy EEA CSF exact chain lost discovery/detail binding")
        receipts = evidence.get("source_receipts") or {}
        if set(receipts) != {"official_calls_index_discovery", "official_exact_call_detail"}:
            raise ExactCSFCall6Error("EEA CSF source receipt set drift")
        for receipt in receipts.values():
            if receipt.get("health_state") != "HEALTHY" or receipt.get("http_status") != 200:
                raise ExactCSFCall6Error("healthy EEA CSF aggregate contains degraded receipt")
            if not re.fullmatch(r"[0-9a-f]{64}", str(receipt.get("raw_sha256") or "")):
                raise ExactCSFCall6Error("healthy EEA CSF receipt lost raw SHA-256")
    elif evidence.get("lkg_required") is not True:
        raise ExactCSFCall6Error("degraded EEA CSF exact chain did not require LKG/reference handling")
    ...
    if "semantic_reconciliation" not in missing or "field_scoped_material_admission" not in missing:
        raise ExactCSFCall6Error("EEA CSF exact evidence weakened downstream gates")
    for key in MATERIAL_FLAGS:
        if evidence.get(key) is not False:
            raise ExactCSFCall6Error(f"EEA CSF exact evidence attempted authorization: {key}")
    if evidence.get("semantic_reconciliation_required") is not True or evidence.get("field_scoped_material_admission_required") is not True:
        raise ExactCSFCall6Error("EEA CSF exact evidence skipped mandatory gates")
    if evidence.get("publication_effect") != "NONE" or evidence.get("canonical_corpus_mutation") is not False:
        raise ExactCSFCall6Error("EEA CSF exact evidence crossed publication boundary")
