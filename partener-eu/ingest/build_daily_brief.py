...
def dossier_url(d: dict[str, Any]) -> str:
    return "#dosar:" + str(d.get("id") or "")
...
def dossier_item(d: dict[str, Any], today: dt.datetime) -> dict[str, Any]:
    ...
    return {
        "id": f"brief-dossier-{d.get('id')}",
        "kind": "DOSSIER",
        "tone": tone,
        "label": label,
        "programme": d.get("programme") or "PROGRAM",
        "title": d.get("title") or "Oportunitate de finanțare",
        "summary": summary[:260],
        "action": action,
        "url": dossier_url(d),
        "dossierId": d.get("id"),
...
def news_item(n: dict[str, Any]) -> dict[str, Any]:
    ...
    labels = {
        "DEADLINE_EXTENDED":"TERMEN PRELUNGIT", "CALL_OPENED":"APEL DESCHIS",
        "GUIDE_MODIFIED":"GHID MODIFICAT", "GUIDE_UPDATED_AFTER_CONSULTATION":"GHID ACTUALIZAT",
        "GUIDE_PUBLISHED":"GHID PUBLICAT", "CONSULTATION_OPENED":"CONSULTARE",
        "CALL_CLOSED":"APEL ÎNCHIS", "RESULTS_PUBLISHED":"REZULTATE",
        "OFFICIAL_UPDATE":"ACTUALIZARE OFICIALĂ",
    }
    ...
    return {
        ...
        "url": "#stire:" + str(n.get("id") or ""), "newsId": n.get("id"),
        "priority": int(n.get("utilityScore") or 60) + 25,
    }
...
def main() -> int:
    ...
    candidates.extend(dossier_item(d, now) for d in payload.get("dossiers") or [] if d.get("status") in {"OPEN","EXPECTED","PUBLIC_CONSULTATION"})
    ...
    for item in candidates:
        ...
        # Prefer variety on homepage; do not allow one programme to occupy all cards.
    ...
    if len(chosen) < 4:
        for item in candidates:
            if any(x["id"] == item["id"] for x in chosen):
                continue
            chosen.append(item)
            if len(chosen) == 4:
                break

    parallel = [d for d in payload.get("dossiers") or [] if d.get("status") == "OPEN" and d.get("id") not in {x.get("dossierId") for x in chosen}]
    ...
    out = {
        ...
        "policy": {"dailyGenerated": True, "decisionProductsOnly": True, "maxCards": 4, "rawIngestionExcluded": True},
    }
