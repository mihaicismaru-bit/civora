...
PARSER_VERSION = "ERASMUS_ACTION_ROUTER_V2"
...
def load_registry(path: Path = DEFAULT_REGISTRY) -> tuple[dict[str, Any], str]:
    ...
    if data.get("registry_id") != "ERASMUS_ACTION_ROUTER_REGISTRY_V2":
...
def _probe_source(source: dict[str, Any], *, timeout: float) -> dict[str, Any]:
    ...
    request = Request(
        url,
        headers={
            "User-Agent": "PARTENER.EU-ErasmusActionRouter/2.0 (+https://partener.eu)",
...
def resolve(
    *,
    ...
    return {
        "schema": "PARTENER_EU_ERASMUS_ACTION_ROUTER_V2",
        "schema_version": "1.0",
        ...
        **flags,
        ...
        "note": (
            ...
            "route guidance, application gateway reachability and action-reference hints cannot authorize OPEN, "
            "CLOSED, deadline, budget or eligibility."
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Resolve official Erasmus+ application routing without authorizing call facts.")
